...
void SV_StopRecord(client_t *cl, qboolean save)
{
	...
	Com_sprintf(cmdline, sizeof(cmdline), "\"%s/apps/%s\" \"%s/%s\"", fs_homepath->string, sv_demoCompletedCmd->string,
...
int FS_DemoWrite(const void *buffer, int len, fileHandleData_t *fh)
{
	...
	if (wb == NULL)
	{
		...
		if (FS_DemoFlush(fh) != oldBufferPos)
	...
	if (len + fh->bufferPos > fh->bufferSize)
	{ // If the buffer is too small to take more data flush it
		if (FS_DemoFlush(fh) != oldBufferPos)
		{ // Fatal file write error
			Com_Printf(CON_CHANNEL_SERVERDEMO, "Demo file write error. Closing file %s\n", fh->name);
			FS_FCloseDemoFile(fh, qfalse);

...
const char *mt_type_names[] = { "empty", "thread", "vector", "notetrack", "anim tree", "small anim tree", "external",
	"temp", "surface", "anim part", "model part", "model part map", "duplicate parts", "model list", "script parse",
	"script string", "class", "tag info", "animscripted", "config string", "debugger string", "vehicle", "generic" };
...
unsigned int MemoryTree::MT_AllocIndex(int numBytes, int type)
{
	...
	assertx(!gScrMemTreeDebugGlob.mt_usage_size[nodeNum], "MT_NodeInfoString( nodeNum ) = %s",
		MT_NodeInfoString(nodeNum));
...
void MemoryTree::MT_FreeIndex(unsigned int nodeNum, int numBytes)
{
	...
	assertx(gScrMemTreeDebugGlob.mt_usage_size[nodeNum] == size, "(MT_NodeInfoString( nodeNum )) = %s",
		MT_NodeInfoString(nodeNum));

...
void SL_FreeString(unsigned int stringValue, RefString *refStr, unsigned int len)
{
	...
	else
	{
		...
		gScrStringGlob.hashTable[prev].status_next =
			(newEntry->status_next & HASH_NEXT_MASK) | (gScrStringGlob.hashTable[prev].status_next & HASH_STAT_MASK);
	}
...
void SL_CheckLeaks()
{
	...
	if (gScrStringDebugGlob)
	{
		if (!gScrStringDebugGlob->ignoreLeaks)
		{
			...
			assertx(!gScrStringDebugGlob->totalRefCount, "gScrStringDebugGlob->totalRefCount = %i",
...
extern "C"
{
	void __cdecl SL_RemoveRefToStringOfSize(unsigned int stringValue, unsigned int len)
	{
		...
		if (InterlockedDecrement((volatile DWORD *)&refStr->data) << 16)
		{
			if (gScrStringDebugGlob)
			{
				assertx(gScrStringDebugGlob->totalRefCount && gScrStringDebugGlob->refCount[stringValue],
					"gScrStringDebugGlob->totalRefCount: %i, stringValue: %i, "
					"gScrStringDebugGlob->refCount[stringValue]: %i, string: '%s'",
					gScrStringDebugGlob->totalRefCount, stringValue, gScrStringDebugGlob->refCount[stringValue],
		...
		else
		{
			...
			do
			{
				...
				InterlockedCompareExchange((DWORD *)&refStr->data, exchange.data, compare.data) != (DWORD)compare.data);
	...
	void __cdecl SL_AddRefToString(unsigned int stringValue)
	{
		...
		if (gScrStringDebugGlob)
		{
			assertx(gScrStringDebugGlob->refCount[stringValue], "%d '%s'", stringValue,
				SL_DebugConvertToString(stringValue));
			assertx(gScrStringDebugGlob->refCount[stringValue] < DEBUG_REFCOUNT_SIZE,
				"SL_DebugConvertToString( stringValue ) = %s", SL_DebugConvertToString(stringValue));
		...
		assertx(refStr->refCount, "string: '%s', refCount: %d", SL_DebugConvertToString(stringValue),
	...
	unsigned int __cdecl SL_FindStringOfSize(const char *str, unsigned int len)
	{
		...
		if (refStr->byteLen != len || memcmp(refStr->str, str, len))
		{
			prev = hash;
			newIndex = entry->status_next & HASH_NEXT_MASK;
			for (newEntry = &gScrStringGlob.hashTable[newIndex]; newEntry != entry;
				 newEntry = &gScrStringGlob.hashTable[newIndex])
			{
				assert((newEntry->status_next & HASH_STAT_MASK) == HASH_STAT_MOVABLE);
				...
				if (refStr->byteLen == len && !memcmp(refStr->str, str, len))
				{
					gScrStringGlob.hashTable[prev].status_next = (newEntry->status_next & HASH_NEXT_MASK)
						| (gScrStringGlob.hashTable[prev].status_next & HASH_STAT_MASK);
					newEntry->status_next =
						(entry->status_next & HASH_NEXT_MASK) | (newEntry->status_next & HASH_STAT_MASK);
	...
	void __cdecl SL_AddUserInternal(RefString *refStr, unsigned int user)
	{
		...
		if (!(user & refStr->user))
		{
			...
			if (gScrStringDebugGlob)
			{
				assertx(gScrStringDebugGlob->refCount[str] < DEBUG_REFCOUNT_SIZE, "SL_DebugConvertToString( str ) = %s",
					SL_DebugConvertToString(str));
				assertx(gScrStringDebugGlob->refCount[str] >= 0, "SL_DebugConvertToString( str ) = %s",
					SL_DebugConvertToString(str));
	...
	unsigned int __cdecl SL_GetStringOfSize(const char *str, unsigned int user, unsigned int len, int type)
	{
		...
		else
		{
			if (entry->status_next & HASH_STAT_MASK)
			{
				...
				for (prev = entry->status_next & HASH_NEXT_MASK;
					 (gScrStringGlob.hashTable[prev].status_next & HASH_NEXT_MASK) != hash;
					 prev = gScrStringGlob.hashTable[prev].status_next & HASH_NEXT_MASK)
					;
				assert(prev != 0);
				newIndex = gScrStringGlob.hashTable->status_next;
				if (!newIndex)
				...
				gScrStringGlob.hashTable[newNext].prev = 0;
				gScrStringGlob.hashTable[prev].status_next =
					newIndex | (gScrStringGlob.hashTable[prev].status_next & HASH_STAT_MASK);
			...
			else
			{
				...
				gScrStringGlob.hashTable[entry->prev].status_next =
					newNext | (gScrStringGlob.hashTable[entry->prev].status_next & HASH_STAT_MASK);
	...
	void __cdecl SL_TransferRefToUser(unsigned int stringValue, unsigned int user)
	{
		...
		if (user & refStr->user)
		{
			assertx(refStr->refCount > 1, "SL_DebugConvertToString( stringValue ) = %s",
				SL_DebugConvertToString(stringValue));
			if (gScrStringDebugGlob)
			{
				assertx(gScrStringDebugGlob->refCount[stringValue], "SL_DebugConvertToString( stringValue, inst ) = %s",
					SL_DebugConvertToString(stringValue));
				InterlockedDecrement((volatile DWORD *)&gScrStringDebugGlob->totalRefCount);
				InterlockedDecrement((volatile DWORD *)&gScrStringDebugGlob->refCount[stringValue]);
			}

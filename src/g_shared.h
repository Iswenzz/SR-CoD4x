...
/* If started at 0x08583BC0, then var_03 points to 0x08583BC8 which is g_playerLastStandLabel - a handler to script
 * function. */
...
#define EF_BREATH           EF_SPINNING     // Characters will not have EF_SPINNING set, hijack for drawing character
breath

#define EF_MELEE_ACTIVE     0x00000800      // (SA) added for client knowledge of melee items held (chair/etc.)

#define EF_SMOKING          EF_MONSTER_EFFECT3  // DHM - Nerve :: ET_GENERAL ents will emit smoke if set // JPW switched
to this after my code change #define EF_CONNECTION       0x00002000      // draw a connection trouble sprite #define
EF_MONSTER_EFFECT2  0x00004000      // show the secondary special effect for this character #define EF_SMOKINGBLACK
EF_MONSTER_EFFECT2  // JPW NERVE -- like EF_SMOKING only darker & bigger #define EF_HEADSHOT         0x00008000      //
last hit to player was head shot #define EF_MONSTER_EFFECT3  0x00010000      // show the third special effect for this
character #define EF_HEADLOOK         0x00020000      // make the head look around*/
...
#define EF_MOVER_STOP       0x10000000      // will push otherwise	// (SA) moved down to make space for one more client
flag
...
#define HORIZONTAL_ALIGN_NOSCALE		5	// uses exact parameters - neither adjusts for safe area nor scales for
screen size #define HORIZONTAL_ALIGN_TO640			6	// scales a real-screen resolution x down into the 0 - 640 range
...
#define VERTICAL_ALIGN_NOSCALE			5	// uses exact parameters - neither adjusts for safe area nor scales for
screen size #define VERTICAL_ALIGN_TO480			6	// scales a real-screen resolution y down into the 0 - 480 range
...
struct entityHandler_t
{
	...
	void(__cdecl *pain)(struct gentity_s *, struct gentity_s *, int, const float *, const int, const float *,
		enum hitLocation_t, const int);
	void(__cdecl *die)(struct gentity_s *, struct gentity_s *, struct gentity_s *, int, int, const int, const float *,
...
extern "C"
{
	...
	void __cdecl player_die(struct gentity_s *self, struct gentity_s *inflictor, struct gentity_s *attacker, int damage,
		int meansOfDeath, int iWeapon, const float *vDir, enum hitLocation_t hitLoc, int psTimeOffset);
	...
	void __cdecl G_LocationalTrace(trace_t *results, const float *start, const float *end, int passEntityNum,
		int contentmask, char *priorityMap);
	...
	void __cdecl Pain_trigger_damage(struct gentity_s *, struct gentity_s *, int, const float *, const int,
		const float *, enum hitLocation_t, const int);
	void __cdecl Die_trigger_damage(struct gentity_s *, struct gentity_s *, struct gentity_s *, int, int, const int,
		const float *, enum hitLocation_t, int);
	...
	void __cdecl G_VehEntHandler_Pain(struct gentity_s *, struct gentity_s *, int, const float *, const int,
		const float *, enum hitLocation_t, const int);
	void __cdecl G_VehEntHandler_Die(struct gentity_s *, struct gentity_s *, struct gentity_s *, int, int, const int,
		const float *, enum hitLocation_t, int);
	void __cdecl G_VehEntHandler_Controller(struct gentity_s *, int *);
	void __cdecl Helicopter_Think(struct gentity_s *);
	void __cdecl Helicopter_Pain(struct gentity_s *, struct gentity_s *, int, const float *, const int, const float *,
		enum hitLocation_t, const int);
	void __cdecl Helicopter_Die(struct gentity_s *, struct gentity_s *, struct gentity_s *, int, int, const int,
		const float *, enum hitLocation_t, int);

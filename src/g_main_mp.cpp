...
#define MAX_REDIRECTDESTINATIONS 4
	...
	*/
	...
	void G_PrintAddRedirect(void (*rd_dest)(const char *, int))
	{
		...
		Com_Error(ERR_FATAL,
			"G_PrintAddRedirect: Out of redirect handles. Increase MAX_REDIRECTDESTINATIONS to add more redirect "
			"destinations");
	}
	...
	*/
		...
		if (*g_log->string)
		{
			...
			if (level.logFile)
			{
				...
				G_LogPrintf("InitGame: %s\\g_logTimeStampInSeconds\\%d\n", serverinfo,
					g_logTimeStampInSeconds->boolean != 0);
			}
			else
			{
	...
	void G_RunFrameForEntity(struct gentity_s *ent)
	{
		...
		assertx(ent->r.maxs[0] >= ent->r.mins[0], "entnum: %d, origin: %g %g %g, classname: %s", ent->s.number,
			ent->r.currentOrigin[0], ent->r.currentOrigin[1], ent->r.currentOrigin[2],
			SL_ConvertToString(ent->classname));
		assertx(ent->r.maxs[1] >= ent->r.mins[1], "entnum: %d, origin: %g %g %g, classname: %s", ent->s.number,
			ent->r.currentOrigin[0], ent->r.currentOrigin[1], ent->r.currentOrigin[2],
			SL_ConvertToString(ent->classname));
		assertx(ent->r.maxs[2] >= ent->r.mins[2], "entnum: %d, origin: %g %g %g, classname: %s", ent->s.number,
			ent->r.currentOrigin[0], ent->r.currentOrigin[1], ent->r.currentOrigin[2],
			SL_ConvertToString(ent->classname));
...
const entityHandler_t entityHandlers[] = { { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0 },
	...
	{ &G_VehEntHandler_Think, NULL, NULL, &G_VehEntHandler_Touch, &G_VehEntHandler_Use, &G_VehEntHandler_Pain,
		&G_VehEntHandler_Die, &G_VehEntHandler_Controller, 0, 0 },

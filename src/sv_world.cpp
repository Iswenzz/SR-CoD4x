...
void CM_AreaEntities_r(unsigned int nodeIndex, areaParms_t *ap)
{
	...
	for (node = &cm_world.sectors[nodeIndex]; node->contents.contentsEntities & ap->contentmask;
...
extern "C"
{
	void __cdecl CM_AddEntityToNode(svEntity_s *ent, uint16_t childNodeIndex)
	{
		...
		for (prevEnt = &cm_world.sectors[childNodeIndex].contents.entities; (unsigned int)*prevEnt - 1 <= entnum;
	...
	qboolean G_ShouldEntitiesClip(moveclip_t *clip, int touchNum, gentity_t *touch)
	{
		if (clip->passEntityNum < 64 && clip->contentmask & CONTENTS_PLAYERCLIP)
		{
			...
			else if (!SV_FriendlyPlayerCanBlock())
			{
				...
				else if (touch->r.ownerNum.isDefined() && touch->r.ownerNum.entnum() < 64
	...
	*/
		...
		int entityNum, int contentmask, int capsule)
	{
		...
		CM_TransformedBoxTrace(trace, start, end, mins, maxs, clipHandle, contentmask, origin, angles);
	...
	__cdecl void SV_ClipMoveToEntity(moveclip_t *clip, svEntity_t *entity, trace_t *trace)
	{
		...
		CM_TransformedBoxTrace(trace, clip->extents.start, clip->extents.end, clip->mins, clip->maxs, clipHandle,
			clip->contentmask, origin, angles);
	...
	*/
	...
	signed int __cdecl SV_SightTraceToEntity(const float *start, const float *mins, const float *maxs, const float *end,
		int entityNum, int contentmask)
	{
		...
		if (touch->r.absmin[0] > bmin[0] || touch->r.absmin[1] > bmin[1] || touch->r.absmin[2] > bmin[2]
			|| bmax[0] > touch->r.absmax[0] || bmax[1] > touch->r.absmax[1] || bmax[2] > touch->r.absmax[2])
		{
			return 0;
		}
		clipHandle = SV_ClipHandleForEntity(touch);
		...
		if (CM_TransformedBoxSightTrace(0, start, end, mins, maxs, clipHandle, contentmask, touch->r.currentOrigin,
				angles))
		{
			return -1;
		}
	...
	void __cdecl SV_SightTrace(int *hitNum, const float *start, const float *mins, const float *maxs, const float *end,
		int passEntityNum0, int passEntityNum1, int contentmask)
	{
		vec3_t temp;
	...
	int __cdecl SV_TracePassed(const float *start, const float *mins, const float *maxs, const float *end,
		int passEntityNum0, int passEntityNum1, int contentmask, int locational, char *priorityMap, int staticmodels)
	{
		vec3_t temp;
		struct sightpointtrace_t spt;
		struct sightclip_t clip;
...
void __cdecl SV_PointTraceToEntity(struct pointtrace_t *clip, svEntity_t *check, trace_t *trace)
{
  ...
  else
  {
	...
	void __cdecl SV_PointTraceToEntity(struct pointtrace_t *clip, svEntity_t *check, trace_t *trace)
	{
		...
		if (!obj)
		{
				...
				&& !CM_TraceBox(&clip->extents, touch->r.absmin, touch->r.absmax, trace->fraction))
			{
				...
				CM_TransformedBoxTrace(trace, clip->extents.start, clip->extents.end, vec3_origin, vec3_origin,
					cliphandle, clip->contentmask, touch->r.currentOrigin, angle);
	...
	void __cdecl SV_PointTraceToEntity_Stub(struct pointtrace_t *clip, trace_t *trace, struct gentity_s *touch,
		DObj *obj, struct DObjTrace_s *objTrace, const float (*entAxis)[3])
	{
		/*
		  int partBits[5];
		  vec3_t localEnd;
	...
	int __cdecl SV_PointSightTraceToEntity(struct sightpointtrace_t *clip, svEntity_t *check)
	{
		...
		for (i = 0; i < 2; ++i)
		{
			if (clip->passEntityNum[i] != 1023
				&& (entnum == clip->passEntityNum[i]
		...
		if (CM_TransformedBoxSightTrace(0, clip->start, clip->end, vec3_origin, vec3_origin, clipHandle,
				clip->contentmask, touch->r.currentOrigin, angles))
		{
			return -1;
		}
		return 0;
	...
	int __cdecl SV_ClipSightToEntity(struct sightclip_t *clip, svEntity_t *check)
	{
		...
		if (clip->passEntityNum[0] != 1023
			&& (entnum == clip->passEntityNum[0]
				|| (touch->r.ownerNum.isDefined() && touch->r.ownerNum.entnum() == clip->passEntityNum[0])))
		{
			return 0;
		}
		if (clip->passEntityNum[1] != 1023
			&& (entnum == clip->passEntityNum[1]
				|| (touch->r.ownerNum.isDefined() && touch->r.ownerNum.entnum() == clip->passEntityNum[1])))
		{
			return 0;
		}
		...
		if (CM_TransformedBoxSightTrace(0, clip->start, clip->end, clip->mins, clip->maxs, clipHandle,
				clip->contentmask, touch->r.currentOrigin, angles))
		{
			return -1;
		}
	...
	void __cdecl SV_Trace(trace_t *results, const float *start, const float *mins, const float *maxs, const float *end,
		IgnoreEntParams *ignoreEntParams, int contentmask, int locational, char *priorityMap, int staticmodels)
	{
		vec3_t temp;

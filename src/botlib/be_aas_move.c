...
void AAS_InitSettings(void)
{
	...
	aassettings.sv_maxbarrier = -0.8
		+ (0.5 * aassettings.sv_gravity * (aassettings.sv_jumpvel / aassettings.sv_gravity)
			* (aassettings.sv_jumpvel / aassettings.sv_gravity));
	// done.
...
void AAS_JumpReachRunStart(aas_reachability_t *reach, vec3_t runstart)
{
	...
	//
	AAS_PredictClientMovement(&move, -1, start, PRESENCE_NORMAL, qtrue, vec3_origin, cmdmove, 1, 2, 0.1,
...
int AAS_PredictClientMovement(struct aas_clientmove_s *move, int entnum, vec3_t origin, int presencetype, int onground,
	vec3_t velocity, vec3_t cmdmove, int cmdframes, int maxframes, float frametime, int stopevent, int stopareanum,
	int visualize)
{
...
void AAS_TestMovementPrediction(int entnum, vec3_t origin, vec3_t dir)
{
	...
	AAS_PredictClientMovement(&move, entnum, origin, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 13, 13, 0.1,

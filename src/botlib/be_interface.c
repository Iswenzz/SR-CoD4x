...
#ifdef _WIN32
	#undef BMAX_PATH // this is an ugly hack, to temporarily ignore the current definition, since it's also defined in
					 // windows.h
	#include <windows.h>
	#undef BMAX_PATH
...
int BotExportTest(int parm0, char *parm1, vec3_t parm2, vec3_t parm3)
{
	...
	if (bot_testhidepos)
	{
		...
		AAS_ClearShownDebugLines();
		hideposarea = AAS_NearestHideArea(0, origin, AAS_PointAreaNum(origin), 0, botlibglobals.goalorigin,
			botlibglobals.goalareanum, TFL_DEFAULT);
  ...
  */
	...
	else
	{
		...
		reachnum = BotGetReachabilityToGoal(origin, newarea, -1, lastgoalareanum, lastareanum, avoidreach,

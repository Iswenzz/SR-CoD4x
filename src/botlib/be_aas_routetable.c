...
void AAS_RT_CalcTravelTimesToGoalArea(int goalarea)
{
	...
	////----(SA)	modified since slime is no longer deadly 	static int tfl = TFL_DEFAULT &
	...
	for (i = 0; i < childcount; i++)
	{
		...
		if (AAS_AreaRouteToGoalArea(filtered_areas[i], (*aasworld).areas[filtered_areas[i]].center, goalarea,
				~RTB_BADTRAVELFLAGS, &travel, &reach))
		{
...
void AAS_RT_WriteRouteTable()
{
	...
	botimport.FS_Write((*aasworld).routetable->children, (*aasworld).routetable->numChildren * sizeof(aas_rt_child_t),
		fp);

	// parents
	botimport.FS_Write(&(*aasworld).routetable->numParents, sizeof(int), fp);
	botimport.FS_Write((*aasworld).routetable->parents, (*aasworld).routetable->numParents * sizeof(aas_rt_parent_t),
		fp);
	...
	botimport.FS_Write((*aasworld).routetable->parentLinks,
		(*aasworld).routetable->numParentLinks * sizeof(aas_rt_parent_link_t), fp);
...
void AAS_RT_BuildRouteTable(void)
{
	...
	area_localinfos =
		(aas_area_buildlocalinfo_t **)AAS_RT_GetClearedMemory(childcount * sizeof(aas_area_buildlocalinfo_t *));
	...
	for (i = 0; i < childcount; i++)
	{
		area_childlocaldata[i] =
			(aas_area_childlocaldata_t *)AAS_RT_GetClearedMemory(sizeof(aas_area_childlocaldata_t));
	...
	while (1)
	{
		...
		// loop around all the parent's visible list, and make them children if they're aren't already assigned to a
		// parent
		for (i = 0; i < localinfo->numvisible; i++)
		{
	...
	for (i = 0; i < num_parents; i++)
	{
		...
		area_parents[i]->visibleParents = (unsigned short int *)AAS_RT_GetClearedMemory(
	...
	//	this would require modification of the aas_area_t structure, so for now, we'll just place them in a global
	// array, for external reference

	//	aasworld->routetable->area_childlocaldata_list = (aas_area_childlocaldata_t **) AAS_RT_GetClearedMemory(
	//(*aasworld).numareas * sizeof(aas_area_childlocaldata_t *) ); 	for (i=0; i<childcount; i++)
	//	{
	...
	//	aasworld->routetable->area_parents_global = (aas_area_parent_t **) AAS_RT_GetClearedMemory( num_parents *
	// sizeof(aas_area_parent_t *) ); 	memcpy( aasworld->routetable->area_parents_global, area_parents, num_parents *
	// sizeof(aas_area_parent_t *) );
	...
	// Convert the data into the correct format
	{
		...
		rt->parentChildren =
			(unsigned short int *)AAS_RT_GetClearedMemory(parentChildrenCount * sizeof(unsigned short int));
		...
		rt->visibleParents =
			(unsigned short int *)AAS_RT_GetClearedMemory(rt->numVisibleParents * sizeof(unsigned short int));
		...
		rt->parentLinks =
			(aas_rt_parent_link_t *)AAS_RT_GetClearedMemory(parentLinkCount * sizeof(aas_rt_parent_link_t));
	...
	botimport.Print(PRT_MESSAGE, "Child Areas: %i\nTotal Parents: %i\nAverage VisAreas: %i\n", (int)childcount,
		num_parents, (int)(childcount / num_parents));
...
#define GETROUTE_NUMROUTES 64
	...
	aas_rt_t *rt;
	static int tfl = TFL_DEFAULT
		& ~(TFL_JUMPPAD | TFL_ROCKETJUMP | TFL_BFGJUMP | TFL_GRAPPLEHOOK | TFL_DOUBLEJUMP | TFL_RAMPJUMP
			| TFL_STRAFEJUMP | TFL_LAVA); //----(SA)	modified since slime is no longer deadly
										  //	static int tfl = TFL_DEFAULT &
	//~(TFL_JUMPPAD|TFL_ROCKETJUMP|TFL_BFGJUMP|TFL_GRAPPLEHOOK|TFL_DOUBLEJUMP|TFL_RAMPJUMP|TFL_STRAFEJUMP|TFL_SLIME|TFL_LAVA);
...
int BotGetReachabilityToGoal(vec3_t origin, int areanum, int entnum, int lastgoalareanum, int lastareanum,
	int *avoidreach, float *avoidreachtimes, int *avoidreachtries, bot_goal_t *goal, int travelflags,
	int movetravelflags);
...
void AAS_RT_ShowRoute(vec3_t srcpos, int srcnum, int destnum)
{
	...
	AAS_ShowAreaPolygons(destnum, 4, qtrue);
	{
		...
		reachnum = BotGetReachabilityToGoal(srcpos, srcnum, -1, lastgoalareanum, lastareanum, avoidreach,
			avoidreachtimes, avoidreachtries, &goal, TFL_DEFAULT | TFL_FUNCBOB, TFL_DEFAULT | TFL_FUNCBOB);
	...
	vec3_t returnPos)
{
	...
	*/
	...
	for (visparents_count = 0, total_parents_checked = 0;
		 visparents_count < MAX_CHECK_VISPARENTS && total_parents_checked < rt->numParents; total_parents_checked++)
	{
	...
	#define GETHIDE_MAX_CHECK_PATHS 15
			...
			for (j = 0; j < GETHIDE_MAX_CHECK_PATHS; j++)
			{
				...
				elapsedTravelTime +=
					AAS_AreaTravelTime(reach->areanum, reach->end, (*aasworld).areas[reach->areanum].center);
				...
				{
					// success ?
					if (!botimport.AICast_VisibleFromPos(destpos, destnum, (*aasworld).areas[pathArea].center, srcnum,
							qfalse))
					{
						// SUCESS !!
				...
				else
				{
					...
					if (!startVisible)
					{ // botimport.AICast_VisibleFromPos( destpos, destnum, reachability[firstreach +
					  // route->reachable_index].end, srcnum, qfalse )) {
						invalidRoute = qtrue;
						break;
					}
				}
		...
		//		if (botimport.AICast_VisibleFromPos( destpos, destnum, (*aasworld).areas[travParent->areanum].center,
		// srcnum, qfalse )) { 			continue;
		//		}
		//

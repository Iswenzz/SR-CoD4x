...
int AAS_Reachability_Jump(int area1num, int area2num)
{
	...
	if (bestdist > 4 && bestdist < maxjumpdistance)
	{
		...
		//
		AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 3, 30, 0.1,
	   ...
	   // create a new reachability link
		...
		{
			lreach->traveltime =
				STARTJUMP_TIME + VectorDistance(bestend, beststart) * 240 / aassettings.sv_maxwalkvelocity;
		} // end if
		  //
...
void AAS_Reachability_JumpPad(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (velocity[0] || velocity[1])
		{
			...
			for (i = 0; i < 20; i++)
			{
				AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse, velocity, cmdmove, 0, 30, 0.1,
					SE_HITGROUND | SE_ENTERWATER | SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD
						| SE_TOUCHTELEPORTER,
					0, qfalse); // qtrue);
...
void AAS_SetWeaponJumpAreaFlags(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!strcmp(classname, "item_armor_body") || !strcmp(classname, "item_armor_combat")
			...
			|| !strcmp(classname, "weapon_sp5") || !strcmp(classname, "weapon_railgun")
				...
				for (l = 0; l < face2->numedges; l++)
				{
					...
					if (abs(edge1num) == abs(edge2num))
					{
						...
						{ //----(SA)	modified since slime is no longer deadly
							...
							//(AREACONTENTS_SLIME | AREACONTENTS_LAVA)) Log_Write("area %d, reach area %d: lava or
						...
						lreach->traveltype = TRAVEL_WALKOFFLEDGE;
						lreach->traveltime =
							STARTWALKOFFLEDGE_TIME + fabs(mid[2] - trace.endpos[2]) * 50 / aassettings.sv_gravity;

...
void AAS_ShowArea(int areanum, int groundfacesonly)
{
	...
	for (n = 0; n < numareaedges; n++)
	{
		...
		botimport.DebugLineShow(debuglines[line], (*aasworld).vertexes[edge->v[0]], (*aasworld).vertexes[edge->v[1]],
			color);
		debuglinevisible[line] = qtrue;
	} // end for*/
...
void AAS_ShowReachability(aas_reachability_t *reach)
{
	...
	if (reach->traveltype == TRAVEL_JUMP || reach->traveltype == TRAVEL_WALKOFFLEDGE)
	{
		...
		//
		AAS_PredictClientMovement(&move, -1, reach->start, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 3, 30, 0.1,
	...
	else if (reach->traveltype == TRAVEL_ROCKETJUMP)
	{
		...
		//
		AAS_PredictClientMovement(&move, -1, reach->start, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 30, 30, 0.1,

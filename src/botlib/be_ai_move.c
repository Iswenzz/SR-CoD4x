...
// Ridah, provide a means of resetting the avoidreach, so if a bot stops moving, they don't avoid the area they were
// heading for
	...
	int movetravelflags)
{
		 ...
		 reachnum = AAS_NextAreaReachability(areanum, reachnum))
	{
	   ...
	   // get the reachability from the number
		...
		//		if (AAS_AreaTravelTimeToGoalArea(areanum, reach.start, goal->areanum, travelflags) + reach.traveltime ==
		// t) 			continue;
...
int BotMovementViewTarget(int movestate, bot_goal_t *goal, int travelflags, float lookahead, vec3_t target)
{
	...
	while (reachnum && dist < lookahead)
	{
		...
		if (reach.traveltype != TRAVEL_JUMPPAD && reach.traveltype != TRAVEL_ELEVATOR
		...
		reachnum = BotGetReachabilityToGoal(reach.end, reach.areanum, -1, ms->lastgoalareanum, lastareanum,
			ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal, travelflags, travelflags);
...
int BotPredictVisiblePosition(vec3_t origin, int areanum, bot_goal_t *goal, int travelflags, vec3_t target)
{
	...
	{
		//
		reachnum = BotGetReachabilityToGoal(end, areanum, -1, lastgoalareanum, lastareanum, avoidreach, avoidreachtimes,
			avoidreachtries, goal, travelflags, travelflags);
		if (!reachnum)
		{
			return qfalse;
		}
...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type)
{
	...
	if (ms->moveflags & MFL_ONGROUND)
	{
		...
		AAS_PredictClientMovement(&move, ms->entitynum, origin, presencetype, qtrue, velocity, cmdmove, cmdframes,
			maxframes, 0.1, stopevent, 0, qfalse); // qtrue);
...
void BotMoveToGoal(bot_moveresult_t *result, int movestate, bot_goal_t *goal, int travelflags)
{
	...
	if (ms->moveflags & (MFL_ONGROUND | MFL_SWIMMING | MFL_AGAINSTLADDER))
	{
		...
		// Ridah, best calc it each frame, especially for Zombie, which moves so slow that we can't afford to take a
		// long route
		//		reachnum = 0;
		// done.
		if (reachnum)
	   ...
	   // or the area changed
				if (ms->lastgoalareanum != goal->areanum || ms->reachability_time < AAS_Time()
					...
					|| ((ms->lasttime > (AAS_Time() - 0.5))
						&& (VectorDistance(ms->origin, ms->lastorigin) < 20 * (AAS_Time() - ms->lasttime))))
				{
					reachnum = 0;
		...
		if (!reachnum)
		{
			...
			reachnum =
				BotGetReachabilityToGoal(ms->origin, ms->areanum, ms->entitynum, ms->lastgoalareanum, ms->lastareanum,
					ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal, travelflags, travelflags);
	...
	else
	{
		...
		for (i = numareas - 1; i >= 0; i--)
		{
			if (AAS_AreaJumpPad(areas[i]))
			{
				...
				foundjumppad = qtrue;
				lastreachnum =
					BotGetReachabilityToGoal(end, areas[i], ms->entitynum, ms->lastgoalareanum, ms->lastareanum,
						ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal, travelflags, TFL_JUMPPAD);
				if (lastreachnum)
				{

...
// Tab Size:		3
	...
	#define RTB_BADTRAVELFLAGS                                                                                         \
		(TFL_JUMPPAD | TFL_ROCKETJUMP | TFL_BFGJUMP | TFL_GRAPPLEHOOK | TFL_DOUBLEJUMP | TFL_RAMPJUMP | TFL_STRAFEJUMP \
...
// (TFL_JUMPPAD|TFL_ROCKETJUMP|TFL_BFGJUMP|TFL_GRAPPLEHOOK|TFL_DOUBLEJUMP|TFL_RAMPJUMP|TFL_STRAFEJUMP|TFL_SLIME|TFL_LAVA)
	...
	#define MAX_VISIBLE_AREAS \
		1024 // going over this limit will result in excessive memory usage, try and keep RANGE low enough so this limit
			 // won't be reached
	#define MAX_LOCALTRAVELTIME \
		60 // use this to tweak memory usage (reduces parent count, increases local count (and cpu usage) - find a
		   // balance)
...
typedef struct
{
	unsigned short int reachable_index; // reachability index (from this area's first reachability link in the world) to
										// head for to get to the destination
...
typedef struct
{
	unsigned short int *areaChildIndexes; // each aas area that is part of the Route-Table has a pointer here to their
	...
	//	int							numParentRoutes;
	//	unsigned char				*parentRoutes;		// reachability to each other parent, as an offset from our
	// first reachability
...
typedef struct aas_parent_link_s
{
	unsigned short int parent;		  // parent we belong to
	unsigned short int childindex;	  // our index in the parent's list of children
	unsigned short int *routeindexes; // for this parent link, list the children that fall under that parent, and their
									  // associated indexes in our localroutes table

...
void AAS_InitPortalCache(void)
{
	...
	(*aasworld).portalcache =
		(aas_routingcache_t **)AAS_RoutingGetMemory((*aasworld).numareas * sizeof(aas_routingcache_t *));
...
void AAS_InitRoutingUpdate(void)
{
	...
	*/
	(*aasworld).areaupdate =
		(aas_routingupdate_t *)AAS_RoutingGetMemory((*aasworld).numareas * sizeof(aas_routingupdate_t));
	//
...
void AAS_WriteRouteCache(void)
{
	...
	buf = (byte *)GetClearedMemory((*aasworld).numareas * 2
		* sizeof(byte)); // in case it ends up bigger than the decompressedvis, which is rare but possible
	...
	routecacheheader.reachcrc = CRC_ProcessString((unsigned char *)(*aasworld).reachability,
...
aas_routingcache_t *AAS_ReadCache(fileHandle_t fp)
{
	...
	cache->reachabilities =
		(unsigned char *)cache + sizeof(aas_routingcache_t) + ((size - sizeof(aas_routingcache_t)) / 3) * 2;
...
int AAS_ReadRouteCache(void)
{
	...
	if (routecacheheader.reachcrc
		!= CRC_ProcessString((unsigned char *)(*aasworld).reachability,
			sizeof(aas_reachability_t) * (*aasworld).reachabilitysize))
	{
		botimport.FS_FCloseFile(fp);
...
__inline void AAS_AddUpdateToList(aas_routingupdate_t **updateliststart, aas_routingupdate_t **updatelistend,
	...
	int *reachnum)
{
	...
	*/
	...
	if (clusternum > 0 && goalclusternum > 0 && clusternum == goalclusternum)
	{
		...
		{
			*reachnum =
				(*aasworld).areasettings[areanum].firstreachablearea + areacache->reachabilities[clusterareanum];
			//
			if (!origin)
			{
...
void AAS_CreateVisibility(void)
{
	...
	for (i = 1; i < (*aasworld).numareas; i++)
	{
		...
		for (j = 1; j < (*aasworld).numareas; j++)
		{
			...
			trace =
				AAS_Trace((*aasworld).areawaypoints[i], NULL, NULL, (*aasworld).areawaypoints[j], -1, CONTENTS_SOLID);
			if (trace.fraction >= 1)
			{
...
#define MAX_HIDEAREA_LOOPS 4000
	...
	if (!(*aasworld).hidetraveltimes)
	{
		(*aasworld).hidetraveltimes =
			(unsigned short int *)GetClearedMemory((*aasworld).numareas * sizeof(unsigned short int));
	}
	else
	{
					...
					{ // now last of all, check that this area is a safe hiding spot
						if (((*aasworld).visCache[nextareanum] == 2)
							|| (!(*aasworld).visCache[nextareanum]
								&& !botimport.AICast_VisibleFromPos(enemyorigin, enemynum,
		...
		for (i = 0; i < numreach; i++, reach++)
		{
			...
			if ((*aasworld).areavisibility[nextareanum])
			{
				...
				{ // now last of all, check that this area is a good attacking spot
					if (((*aasworld).visCache[nextareanum] == 2)
						|| (!(*aasworld).visCache[nextareanum] && (count += 10)
							&& // we are about to use lots of CPU time
							botimport.AICast_CheckAttackAtPos(srcnum, enemynum, (*aasworld).areawaypoints[nextareanum],
								qfalse, qfalse)))
					{
						(*aasworld).visCache[nextareanum] = 2;

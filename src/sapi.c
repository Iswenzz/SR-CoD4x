...
static uint64_t ParseSteam3ID(const char* h)
{
	...
	if (cFieldConverted == EOF || cFieldConverted != 3 || extracheck
		|| (universe > 4 && (universe < 32 || universe >= 40)) || universe < 1 || accounttypei < 1 || accounttypei > 10)
	{
		return 0;
	}

	uint64_t steamid =
		((uint64_t)universe << 56) | ((uint64_t)accounttypei << 52) | ((uint64_t)instance << 32) | (uint64_t)accountid;
	return steamid;
}
...
*/
/*
Copied from libtomcrypt so here a sleep can be included for less blocking the main thread while this function is
operating
*/
int pkcs_5_alg2_200sleep(const unsigned char* password, unsigned long password_len, const unsigned char* salt,
	unsigned long salt_len, int iteration_count, int hash_idx, unsigned char* out, unsigned long* outlen)
{
	int err, itts, i;
	ulong32 blkno;
...
static const unsigned char playerid_salt[] = { ,
...
uint64_t SV_SApiGUID2PlayerID(const char* guid)
{
	...
	if (pkcs_5_alg2(diggest2, sizeof(diggest2), playerid_salt, sizeof(playerid_salt), 100, hash_idx,
			(unsigned char*)&accountid, &outlen)
		!= CRYPT_OK) // This function is special. It sleeps
	{
	...
	uint64_t steamid =
		((uint64_t)universe << 56) | ((uint64_t)accounttype << 52) | ((uint64_t)instance << 32) | accountid;
	return steamid;
}
...
void SV_InitSApi()
{
	cvar_t* sv_noauth = Cvar_RegisterBool("sv_noauth", qfalse, CVAR_INIT,
		"Start the server without client authentication. Unofficial clients will be able to join.");
	if (sv_noauth->boolean)
	{
		Com_PrintMessage(CON_CHANNEL_SERVER,
			"^1Warning: Running without client authentication. Unofficial clients will be able to join.\n",
			MSG_WARNING);
		return;
	}
	...
	sv_usesteam64id =
		Cvar_RegisterBool("sv_usesteam64id", qtrue, CVAR_ARCHIVE, "Display and log Steam64 id in most commands");

	hmodule = Sys_LoadLibrary("steam_api" DLL_EXT);
	if (hmodule == NULL)
	{
		Sys_LoadLibraryError(errormsg, sizeof(errormsg));
		SApi_PrintError("steam_api" DLL_EXT
						" not found or it was not possible to load. Error is: %s. Steam is not going to work.\n",
			errormsg);
		return;
	}
	char* buf;
	...
	if (len > 0)
	{
		...
		if (bottomval == topval)
		{
			...
			SApi_PrintError(
				"Steam library is too old and causes incompatibilities with players who are using Steam with a cod4 "
				"license.\n");
			return;
		}
		free(buf);
	...
	{
		sv_steamgroup =
			Cvar_RegisterString("sv_steamgroup", "", CVAR_ARCHIVE | CVAR_LATCH, "Steam group this server belongs to");
	}
...
qboolean SV_SApiGetGroupMemberStatusByClientNum(int clnum, uint64_t groupid, uint64_t reference,
	void (*callback)(int clientnum, uint64_t steamid, uint64_t groupid, uint64_t reference, bool m_bMember,
		bool m_bOfficer))
{
...
uint32_t SV_SApiGetAuthenticationTicket(unsigned char* data, int* buflen, uint64_t* steamid)
{
	...
	{
		return sapi_imp.GetAuthenticationTicket(data, buflen,
			steamid); // To authenticate this server towards other servers
	}
	return 0;
}

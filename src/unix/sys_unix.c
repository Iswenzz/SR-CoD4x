...
void Sys_ReplaceProcess(char *cmdline)
{
	...
	else
	{
		Com_PrintError(CON_CHANNEL_SYSTEM,
			"Sys_ReplaceProcess: Exceeded limit of 256 commandline arguments.\nCommandline is: %s\n", cmdline);
	}
...
{
	if (mprotect((char *)startoffset - ((int)startoffset % getpagesize()), len + (len % getpagesize()),
...
char **Sys_ListFiles(const char *directory, const char *extension, char *filter, int *numfiles, qboolean wantsubs)
{
	...
	while ((d = readdir(fdir)) != NULL)
	{
		...
		if ((dironly && !(st.st.st_mode & S_IFDIR)) || (!dironly && (st.st.st_mode & S_IFDIR)))
			continue;
...
int main(int argc, char *argv[])
{
	...
	{ // warn user that he/she's operating as a privliged user
...
qboolean Sys_CreateNewThread(void *(*ThreadMain)(void *), threadid_t *tid, void *arg)
{
	...
	if (err != 0)
	{
		...
		Com_PrintError(CON_CHANNEL_SYSTEM,
			"pthread_attr_setdetachstate(): Thread creation failed with the following error: %s\n", strerror(errno));
		return qfalse;
	}
	...
	if (err != 0)
	{
		pthread_attr_destroy(&tattr);
		Com_PrintError(CON_CHANNEL_SYSTEM,
			"pthread_attr_setstacksize(): Thread creation failed with the following error: %s\n", strerror(errno));
		return qfalse;
	}
...
BOOL __cdecl _ReadFileEx(HANDLE handle, void *lpBuffer, int nNumberOfBytesToRead, struct _OVERLAPPED *lpOverlapped,
	void(__stdcall *lpCompletionRoutine)(long unsigned int, long unsigned int, struct _OVERLAPPED *))
{
	sLastError = 0;
	hObject_t *hObject = (hObject_t *)handle;
	if (hObject->type != 'File')
...
HANDLE __cdecl _CreateFileA(char *lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, void *lpSecurityAttributes,

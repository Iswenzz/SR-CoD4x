...
const char *var_typename[] = { "undefined", "object", "string", "localized string", "vector", "float", "int", "codepos",
	"precodepos", "function", "stack", "animation", "developer codepos", "include codepos", "thread", "thread",
	"thread", "thread", "struct", "removed entity", "entity", "array", "removed thread" };
...
struct scr_classStruct_t gScrClassMap[] = { { 0u, 0u, 'e', "entity" }, { 0u, 0u, 'h', "hudelem" },
	{ 0u, 0u, 'p', "pathnode" }, { 0u, 0u, 'v', "vehiclenode" }, { 0u, 0u, 'd', "dynentity" } };
...
extern "C"
{
	...
	void __regparm2 MakeVariableExternal(unsigned int index, VariableValueInternal *parentValue)
	{
		...
		assert((entryValue->w.status & VAR_STAT_MASK) == VAR_STAT_MOVABLE
			|| (entryValue->w.status & VAR_STAT_MASK) == VAR_STAT_HEAD);
		...
		if ((entryValue->w.status & VAR_STAT_MASK) == VAR_STAT_HEAD)
		{
			...
			if (oldEntry != entry)
			{
				...
				{
					gScrVarGlob
						.variableList[gScrVarGlob.variableList[oldPrevSiblingIndex + VARIABLELIST_CHILD_BEGIN].hash.id
				...
				{
					gScrVarGlob
						.variableList[gScrVarGlob.variableList[prevSiblingIndex + VARIABLELIST_CHILD_BEGIN].hash.id
							+ VARIABLELIST_CHILD_BEGIN]
						.nextSibling = oldIndex;
				}
		...
		else
		{
			...
			{
				assert((oldEntryValue->w.status & VAR_STAT_MASK) == VAR_STAT_MOVABLE
					|| (oldEntryValue->w.status & VAR_STAT_MASK) == VAR_STAT_HEAD);
				prev = &oldEntry->hash;
	...
	unsigned int __cdecl FindVariableIndexInternal2(unsigned int name, unsigned int index)
	{
		...
		assertx(index < VARIABLELIST_CHILD_SIZE, "index doesn't index VARIABLELIST_CHILD_SIZE - %i not in [0, %i]",
			index, VARIABLELIST_CHILD_SIZE);

		entry = &list[index].hash;
		assertx(entry->id < VARIABLELIST_CHILD_SIZE,
			"entry->id doesn't index VARIABLELIST_CHILD_SIZE - %i not in [0, %i]", entry->id, VARIABLELIST_CHILD_SIZE);
	...
	void __cdecl FreeChildValue(unsigned int parentId, unsigned int id)
	{
		...
		if (prevSiblingIndex)
		{
			gScrVarGlob
				.variableList[gScrVarGlob.variableList[prevSiblingIndex + VARIABLELIST_CHILD_BEGIN].hash.id
					+ VARIABLELIST_CHILD_BEGIN]
				.nextSibling = nextSiblingIndex;
		}
		else
		{
			assert(!gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN].hash.id);
			gScrVarGlob.variableList[parentId + 1].nextSibling =
				gScrVarGlob.variableList[nextSiblingIndex + VARIABLELIST_CHILD_BEGIN].hash.id;
		}
		if (nextSiblingIndex)
		{
		...
		else
		{
			...
			assert(!gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN].hash.id);
			gScrVarGlob.variableList[parentIndex + VARIABLELIST_PARENT_BEGIN].hash.u.prev =
				gScrVarGlob.variableList[prevSiblingIndex + VARIABLELIST_CHILD_BEGIN].hash.id;
		}
		entryValue->w.status = 0;
		entryValue->u.next = gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN].u.next;
		entry->u.prev = 0;
		gScrVarGlob.variableList[gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN].u.next + VARIABLELIST_CHILD_BEGIN]
			.hash.u.prev = index;
	...
	void __cdecl FreeVariable(unsigned int id)
	{
		...
		entry->u.prev = 0;
		gScrVarGlob.variableList[gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN].u.next + VARIABLELIST_PARENT_BEGIN]
			.hash.u.prev = index;
	...
	{
		assert(
			(gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + parentId].w.status & VAR_STAT_MASK) != VAR_STAT_FREE);
	...
	unsigned int __regparm3 GetNewVariableIndexInternal3(unsigned int parentId, unsigned int name, unsigned int index)
	{
		...
		if (type)
		{
			if (type == VAR_STAT_HEAD)
			{
				...
				else
				{
					...
					gScrVarGlob
						.variableList[gScrVarGlob.variableList[oldEntryValue->hash.u.prev + VARIABLELIST_CHILD_BEGIN]
										  .hash.id
							+ VARIABLELIST_CHILD_BEGIN]
						.u.next = entry->u.next;
					gScrVarGlob.variableList[entry->u.next + VARIABLELIST_CHILD_BEGIN].hash.u.prev =
						oldEntryValue->hash.u.prev;
					...
					gScrVarGlob
						.variableList[gScrVarGlob.variableList[oldEntryValue->hash.u.prev + VARIABLELIST_CHILD_BEGIN]
										  .hash.id
							+ VARIABLELIST_CHILD_BEGIN]
						.nextSibling = entry->v.next;
					gScrVarGlob.variableList[entryValue->nextSibling + VARIABLELIST_CHILD_BEGIN].hash.u.prev =
						entry->v.next;
					entryValue->w.status &= ~VAR_STAT_MASK;
			...
			else
			{
				...
				else
				{
					...
					newEntryValue = entry;
					gScrVarGlob
						.variableList[gScrVarGlob.variableList[newEntry->hash.u.prev + VARIABLELIST_CHILD_BEGIN].hash.id
							+ VARIABLELIST_CHILD_BEGIN]
						.u.next = entry->u.next;
					gScrVarGlob.variableList[entry->u.next + VARIABLELIST_CHILD_BEGIN].hash.u.prev =
						newEntry->hash.u.prev;
				}
				...
				if (type == VAR_STAT_MOVABLE)
				{
					...
					assert((gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN + prevId].w.status & VAR_STAT_MASK)
							== VAR_STAT_MOVABLE
						|| (gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN + prevId].w.status & VAR_STAT_MASK)
							== VAR_STAT_HEAD);

					while (gScrVarGlob.variableList[prevId + VARIABLELIST_CHILD_BEGIN].v.next != index)
					{
						prevId = gScrVarGlob
									 .variableList[gScrVarGlob.variableList[prevId + VARIABLELIST_CHILD_BEGIN].v.next
										 + VARIABLELIST_CHILD_BEGIN]
									 .hash.id;
						assert((gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN + prevId].w.status & VAR_STAT_MASK)
								== VAR_STAT_MOVABLE
		...
		else
		{
			...
			assertx(!gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN + prev].hash.id
					|| (gScrVarGlob
							   .variableList[VARIABLELIST_CHILD_BEGIN
								   + gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN + prev].hash.id]
							   .w.status
						   & VAR_STAT_MASK)
						== VAR_STAT_FREE,
				va("%d, %d, %d", prev, gScrVarGlob.variableList[prev + VARIABLELIST_CHILD_BEGIN].hash.id,
					gScrVarGlob
							.variableList[gScrVarGlob.variableList[prev + VARIABLELIST_CHILD_BEGIN].hash.id
								+ VARIABLELIST_CHILD_BEGIN]
							.w.status
						& VAR_STAT_MASK));
	...
	{
		return gScrVarGlob.variableList[GetArrayVariableIndex(parentId, unsignedValue) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
	...
	void __regparm2 CopyArray(unsigned int parentId, unsigned int newParentId)
	{
		...
		if (id)
		{
			while (1)
			{
				...
				newEntryValue = &gScrVarGlob.variableList[gScrVarGlob
															  .variableList[GetVariableIndexInternal(newParentId,
				...
				if (type == VAR_POINTER)
				{
					if (VAR_TYPE((&gScrVarGlob.variableList[entryValue->u.u.pointerValue + VARIABLELIST_PARENT_BEGIN]))
						== VAR_ARRAY)
					{
	...
	VariableValue __cdecl Scr_EvalVariableEntityField(unsigned int entId, unsigned int fieldName)
	{
		...
		if (fieldId)
		{
			value = GetEntityFieldValue(entValue->w.classnum >> VAR_NAME_BITS, entValue->u.o.u.entnum & UNK_ENTNUM_MASK,
...
// Fix for weird GNU-GCC ABI
	...
	VariableValue Scr_FindVariableField(unsigned int parentId, unsigned int name)
	{
		...
		assert((entryValue->w.status & VAR_STAT_MASK) != VAR_STAT_FREE);
		assert(IsObject(entryValue));
		assert((VAR_TYPE(entryValue) >= FIRST_OBJECT && VAR_TYPE(entryValue) < FIRST_NONFIELD_OBJECT)
			|| VAR_TYPE(entryValue) >= FIRST_DEAD_OBJECT);
...
// Fix for weird GNU-GCC ABI
	...
	unsigned int __cdecl Scr_EvalArrayRef(unsigned int parentId)
	{
		...
		else
		{
			...
			assert((entValue->w.classnum >> VAR_NAME_BITS) < CLASS_NUM_COUNT);
			fieldId =
				FindArrayVariable(gScrClassMap[entValue->w.classnum >> VAR_NAME_BITS].id, gScrVarPub.entFieldName);
			if (fieldId)
			{
				varValue =
					GetEntityFieldValue(entValue->w.classnum >> VAR_NAME_BITS, entValue->u.o.u.entnum & UNK_ENTNUM_MASK,
						gScrVarGlob.variableList[fieldId + VARIABLELIST_CHILD_BEGIN].u.u.entityOffset);
				if (varValue.type)
				{
					if (varValue.type == VAR_POINTER
						&& !gScrVarGlob.variableList[varValue.u.pointerValue + VARIABLELIST_PARENT_BEGIN].u.o.refCount)
					{
					...
					assert((varValue.type != VAR_POINTER)
						|| !gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + varValue.u.pointerValue].u.o.refCount);

					parentValue = 0;
					goto add_array;
				}
			}
			parentValue = &gScrVarGlob.variableList[GetNewVariable(gScrVarPub.entId, gScrVarPub.entFieldName)
				+ VARIABLELIST_CHILD_BEGIN];
		}
	...
	void __cdecl ClearArray(unsigned int parentId, VariableValue *value)
	{
		...
		else
		{
			...
			if (varValue.type == VAR_POINTER
				&& !gScrVarGlob.variableList[varValue.u.intValue + VARIABLELIST_PARENT_BEGIN].u.o.refCount)
			{
	...
	void __cdecl RemoveNextVariable(unsigned int parentId)
	{
		...
		id = gScrVarGlob.variableList[parentId + VARIABLELIST_PARENT_BEGIN].nextSibling;
		assert(id != 0);
		index = FindVariableIndexInternal(parentId,
			gScrVarGlob.variableList[id + VARIABLELIST_CHILD_BEGIN].w.classnum >> VAR_NAME_BITS);
		assert(index != 0);
	...
	{
		return gScrVarGlob
			.variableList[FindVariableIndexInternal(parentId, id + SL_MAX_STRING_INDEX) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
		...
		else
		{
			...
			if (VAR_TYPE(parentValue) == VAR_ENTITY)
			{
				...
				if (fieldId)
				{
					++value;
					value->type = VAR_UNDEFINED;
					SetEntityFieldValue(classnum, SCR_GET_ENTITY_FROM_ENTCLIENT(parentValue->u.o.u.entnum),
						gScrVarGlob.variableList[fieldId + VARIABLELIST_CHILD_BEGIN].u.u.intValue, value);
				}
			}
		}
	}
	...
	{
		return gScrVarGlob.variableList[FindVariableIndexInternal(parentId, unsignedValue) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
	...
	{
		return gScrVarGlob.variableList[GetVariableIndexInternal(parentId, unsignedValue) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
	...
	{
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.status & VAR_STAT_MASK)
			== VAR_STAT_EXTERNAL);
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.type & VAR_MASK) >= VAR_THREAD
			&& (gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.type & VAR_MASK) <= VAR_CHILD_THREAD);
		...
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.status & VAR_STAT_MASK)
			== VAR_STAT_EXTERNAL);
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.type & VAR_MASK) >= VAR_THREAD
			&& (gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.type & VAR_MASK) <= VAR_TIME_THREAD);
		return threadId;
	}
	...
	{
		assert(
			(gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.status & VAR_STAT_MASK) != VAR_STAT_FREE);
		assert(((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.type & VAR_MASK) >= VAR_THREAD)
			&& ((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.type & VAR_MASK)
				<= VAR_CHILD_THREAD));
	...
	{
		return gScrVarGlob.variableList[GetNewVariableIndexInternal(parentId, unsignedValue) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
	...
	unsigned int __cdecl GetObjectVariable(unsigned int parentId, unsigned int id)
	{
		...
		return gScrVarGlob
			.variableList[GetVariableIndexInternal(parentId, id + SL_MAX_STRING_INDEX) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
	...
	void __cdecl Scr_KillThread(unsigned int parentId)
	{
		...
		if (id)
		{
			for (selfNameId = FindObject(id);; RemoveObjectVariable(selfNameId, name - SL_MAX_STRING_INDEX))
			{
				...
				assert(
					VAR_TYPE((&gScrVarGlob.variableList[VARIABLELIST_CHILD_BEGIN + notifyListEntry])) == VAR_POINTER);
	...
	unsigned int __cdecl GetNewObjectVariable(unsigned int parentId, unsigned int id)
	{
		assert(VAR_TYPE((&gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + parentId])) == VAR_ARRAY);
		return gScrVarGlob
			.variableList[GetNewVariableIndexInternal(parentId, id + SL_MAX_STRING_INDEX) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
	...
	unsigned int __cdecl GetSafeParentLocalId(unsigned int threadId)
	{
		...
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId].w.status & VAR_STAT_MASK)
			== VAR_STAT_EXTERNAL);
		assert(VAR_TYPE((&gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId])) >= VAR_THREAD
			&& VAR_TYPE((&gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + threadId])) <= VAR_CHILD_THREAD);
	...
	void __cdecl Scr_FreeEntityNum(int entnum, unsigned int classnum)
	{
		...
		if (gScrVarPub.bInited)
		{
			...
			if (entnumId)
			{
				...
				// assert(SCR_GET_ENTITY_FROM_ENTCLIENT(entryValue->u.o.u.entnum) == entnum -
				// (SCR_GET_CLIENT_FROM_ENTCLIENT(entryValue->u.o.u.entnum) * MAX_LOCAL_CENTITIES));
	...
	{
		return gScrVarGlob.variableList[GetNewArrayVariableIndex(parentId, unsignedValue) + VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
...
#define UNKNOWN_TYPE 0
	...
	{
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + startLocalId].w.status & VAR_STAT_MASK)
			== VAR_STAT_EXTERNAL);
		assert(
			(gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + startLocalId].w.type & VAR_MASK) == VAR_TIME_THREAD);
	...
	{
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + startLocalId].w.type & VAR_STAT_MASK)
			== VAR_STAT_EXTERNAL);
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + startLocalId].w.type & VAR_MASK)
			== VAR_NOTIFY_THREAD);
		assert((gScrVarGlob.variableList[VARIABLELIST_PARENT_BEGIN + startLocalId].w.notifyName >> VAR_NAME_BITS)
			< (1 << 16));
	...
	static unsigned int GetNewVariableIndexReverseInternal2(unsigned int parentId, unsigned int name,
		unsigned int index)
	{
		VariableValueInternal *parentValue;
		unsigned int siblingId;
	...
	unsigned int __cdecl GetNewObjectVariableReverse(unsigned int parentId, unsigned int id)
	{
		...
		return gScrVarGlob
			.variableList[GetNewVariableIndexReverseInternal(parentId, id + SL_MAX_STRING_INDEX)
				+ VARIABLELIST_CHILD_BEGIN]
			.hash.id;
	}
	...
	static void SetVariableEntityFieldValue(unsigned int entId, unsigned int fieldName, VariableValue *value)
	{
		...
		if (!fieldId
			|| !SetEntityFieldValue(entValue->w.classnum >> VAR_NAME_BITS,
				SCR_GET_ENTITY_FROM_ENTCLIENT(entValue->u.o.u.entnum),
			...
			if (infoArray)
			{
				...
				while (i < num)
				{
					...
					Com_Printf(CON_CHANNEL_PARSERSCRIPT, "count: %d, var usage: %d, endon usage: %d\n", count,
						(signed int)info.varUsage, (signed int)info.endonUsage);
				...
				for (classnum = 0; classnum < CLASS_NUM_COUNT; ++classnum)
				{
					if (gScrClassMap[classnum].entArrayId)
					{
						info.varUsage = 0.0;
						count = 0;
						for (entId = FindFirstSibling(gScrClassMap[classnum].entArrayId); entId;
							 entId = FindNextSibling(entId))
						{
							++count;
							if (VAR_TYPE((&gScrVarGlob.variableList[entId + VARIABLELIST_CHILD_BEGIN])) == VAR_POINTER)
							{
								info.varUsage += Scr_GetObjectUsage(
									gScrVarGlob.variableList[entId + VARIABLELIST_CHILD_BEGIN].u.next);
							}
						}
	...
	void Scr_DumpScriptVariables(bool spreadsheet, bool summary, bool total, bool functionSummary, bool lineSort,
		const char *fileName, const char *functionName, int minCount)
	{
		const char *pos;
					...
					for (index = 0; index < maxnum; ++index)
					{
						...
						if (pos)
						{
							...
							if (!fileName || Scr_PrevCodePosFileNameMatches(pos, fileName))
							{
								...
								if (!functionName
									|| (pInfo->functionName && Q_stristr(pInfo->functionName, functionName)))
								{
									pInfo->pos = pos;
							...
							if (infoArray[i].varUsage)
							{
								...
								if (pInfo->varUsage >= minCount)
								{
									...
									if (spreadsheet)
									{
										...
										Scr_PrintPrevCodePosSpreadSheet(CON_CHANNEL_DONT_FILTER, pInfo->pos, summary,
											functionSummary);
									}
									else
									{
	...
	void __cdecl Scr_InitVariables()
	{
		...
		gScrVarGlob.variableList = (VariableValueInternal *)_PMem_AllocNamed(
			sizeof(VariableValueInternal) * variableCount, 0x80u, 4u, 1u, "ScriptVars", TRACK_SCRIPT);
		...
		if (gScrVarDebugPub)
		{
			gScrVarDebugPub->leakCount =
				(int *)_PMem_AllocNamed(variableCount * sizeof(int), 4u, 4u, 1u, "ScriptVars", TRACK_SCRIPT);
		...
		if (gScrVarDebugPub)
		{
			gScrVarDebugPub->extRefCount = (uint16_t *)_PMem_AllocNamed(VARIABLELIST_PARENT_SIZE * sizeof(uint16_t), 4u,
				4u, 1u, "ScriptVars", TRACK_SCRIPT);
		...
		if (gScrVarDebugPub)
		{
			gScrVarDebugPub->varUsage =
				(const char **)_PMem_AllocNamed(variableCount * sizeof(int), 4u, 4u, 1u, "ScriptVars", TRACK_SCRIPT);
...
#define _LEAKED_ 0
		...
		for (index = 1; index < (signed int)(end - begin); ++index)
		{
			...
			case VAR_STAT_MOVABLE:
				Com_Printf(CON_CHANNEL_PARSERSCRIPT, "move: %d -> %d\n", entry->hash.id + begin,
					gScrVarGlob.variableList[value->v.next + begin].hash.id + begin);
				break;
			case VAR_STAT_HEAD:
			...
			Com_Printf(CON_CHANNEL_PARSERSCRIPT, "%d -> %d\n", entry->hash.id + begin,
				gScrVarGlob.variableList[value->nextSibling + begin].hash.id);
			Com_Printf(CON_CHANNEL_PARSERSCRIPT, "%d <- %d\n", entry->hash.id + begin,
				gScrVarGlob.variableList[entry->hash.u.prev + begin].hash.id);
			continue;
		}
	}
	...
	void Scr_UnmatchingTypesError(VariableValue *value1, VariableValue *value2)
	{
		...
		else
		{
			...
			error_message =
				va("pair '%s' and '%s' has unmatching types '%s' and '%s'", SL_ConvertToString(value1->u.intValue),
					SL_ConvertToString(value2->u.intValue), var_typename[type1], var_typename[type2]);
		}
	...
	void __cdecl Scr_EvalEquality(VariableValue *value1, VariableValue *value2)
	{
		...
		case VAR_ANIMATION:
			...
			break;
		case VAR_BEGIN_REF:
			if (!(((gScrVarGlob.variableList[value1->u.intValue + VARIABLELIST_CHILD_BEGIN].w.type & VAR_MASK)
						  == VAR_ARRAY
					  || (gScrVarGlob.variableList[value2->u.intValue + VARIABLELIST_CHILD_BEGIN].w.type & VAR_MASK)
						  == VAR_ARRAY)
					&& !gScrVarPub.evaluate))
			{
				value1->type = VAR_INTEGER;

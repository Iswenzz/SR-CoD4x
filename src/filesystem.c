			...
			if (search->pack->hashTable[hash])
			{
				...
				do
				{
					...
					if (!FS_FilenameCompare(pakFile->name, filename))
					{
						...
						err =
							unzGetCurrentFileInfo(pak->handle, &file_info, netpath, sizeof(netpath), NULL, 0, NULL, 0);
						if (err != UNZ_OK)
						{
							Com_PrintWarning(CON_CHANNEL_FILES, "Read error in Zip-file: %s\n", pak->pakFilename);
							return 1;
		...
		if (search->pack->hashTable[hash])
		{
			...
			do
			{
				...
				if (!FS_FilenameCompare(pakFile->name, filename))
				{
					...
					err = unzGetCurrentFileInfo(fsh[*file].handleFiles.file.z, &file_info, netpath, sizeof(netpath),
						NULL, 0, NULL, 0);
					if (err != UNZ_OK)
					{
						Com_PrintWarning(CON_CHANNEL_FILES, "Read error in Zip-file: %s\n", pak->pakFilename);
...
static pack_t *FS_LoadZipFile(char *zipfile, const char *basename)
{
	...
	if (fs_checksumFeed)
		pack->pure_checksum =
			Com_BlockChecksumKey32(fs_headerLongs, 4 * fs_numHeaderLongs, LittleLong(fs_checksumFeed));
	else
...
void FS_InitFilesystem()
{
	...
	//    Com_Error(ERR_DROP, "Couldn't load %s.  Make sure Call of Duty is run from the correct folder.",
	//    "fileSysCheck.cfg");
...
qboolean FS_GameDirDomainFunc(const char *cvar_name, const char *fs_gamestring)
{
	...
	if (Q_stricmpn(fs_gamestring, "mods", 4) || strlen(fs_gamestring) < 6
		|| (fs_gamestring[4] != '/' && fs_gamestring[4] != '\\') || strstr(fs_gamestring, "..")
...
static char *g_languages[] = { "english", "french", "german", "italian", "spanish", "british", "russian", "polish",
...
void FS_InitCvars()
{
	...
	fs_gameDirVar =
		Cvar_RegisterString("fs_game", "", 28, "Game data directory. Must be \"\" or a sub directory of 'mods/'.");
...
void FS_AddIwdFilesForGameDirectory(const char *path, const char *dir)
{
	...
	for (i = 0; i < numfiles; i++)
	{
		if (!Q_strncmp(sorted[i], "          ", 10))
		{
			...
			{
				Com_PrintWarning(CON_CHANNEL_FILES,
					"WARNING: Localized assets iwd file %s/%s/%s has invalid name (no language specified). Proper "
			...
			{
				Com_PrintWarning(CON_CHANNEL_FILES,
					"WARNING: Localized assets iwd file %s/%s/%s has invalid name (bad language name specified). "
					"Proper naming convention is: localized_[language]_iwd#.iwd\n",
					path, dir, sorted[i]);
				if (!languagesListed)
				{
		...
		else
		{
			if (!Q_stricmp(dir, BASEGAME) && !Q_stricmp(path, fs_basepath->string) && Q_stricmpn(sorted[i], "iw_", 3)
				&& Q_stricmpn(sorted[i], "jcod4x_", 7) && Q_stricmpn(sorted[i], "xbase_", 6))
			{
...
char __cdecl FS_SanitizeFilename(const char *filename, char *sanitizedName, int sanitizedNameSize)
{
	...
	while (filename[srcIndex])
	{
		...
		if (filename[srcIndex] != '.'
			|| (filename[srcIndex + 1] != 0 && filename[srcIndex + 1] != '/' && filename[srcIndex + 1] != '\\'))
		{

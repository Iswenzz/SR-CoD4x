	...
	while (msg->readcount < msg->cursize)
	{
		...
		default:
			...
			Com_Printf(CON_CHANNEL_DONT_FILTER,
				"Not a valid source rcon packet from: %s received. Type: %d - Closing connection\n",
...
void HL2Rcon_StatusCommand()
{
	...
	Com_Printf(CON_CHANNEL_DONT_FILTER, "players : %d humans, %d bots (%d/%d)\n\n", humans, bots,
		sv_maxclients->integer, humans + bots);
	...
	for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++)
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER, "# %d \"%s\" %s %s %d %d %s %d %s\n", i, cl->name, psti, timestr, ping, 0,
			state, cl->rate, NET_AdrToString(&cl->netchan.remoteAddress));
	}

...
/* MenuDef_t fields. */
	...
	#define WRITE_MENUDEF_FIELD_FLAG(str_key, int_condition) \
		WRITE_MENU_FIELD_COND((int_condition), MENUDEF_INDENT, "", (str_key))

	#define WRITE_MENUDEF_FIELD_EXPRESSION(str_key, expression)                                           \
		WRITE_MENU_FIELD_COND((expression).entries_count, MENUDEF_INDENT, "%s ( %s );", "exp", (str_key), \
	...
	#define WRITE_MENUDEF_FIELD_KEY(key_hndl)                                                          \
		WRITE_MENU_FIELD_COND((key_hndl), MENUDEF_INDENT, "\"%c\" { %s }", "execKey", (key_hndl)->key, \
			action_optimize((key_hndl)->action))

	#define WRITE_MENUDEF_FIELD_VEC4(str_key, vec4_val)                                                  \
		WRITE_MENU_FIELD_COND(1, MENUDEF_INDENT, "%g %g %g %g", (str_key), (vec4_val)[0], (vec4_val)[1], \
			(vec4_val)[2], (vec4_val)[3])

	#define WRITE_MENUDEF_FIELD_RECT(str_key, rect_val)                                                             \
		WRITE_MENU_FIELD_COND(((rect_val).x || (rect_val).y || (rect_val).w || (rect_val).h || (rect_val).horzAlign \
								  || (rect_val).vertAlign),                                                         \
			MENUDEF_INDENT, "%g %g %g %g %s %s", (str_key), (rect_val).x, (rect_val).y, (rect_val).w, (rect_val).h, \
			find_word(g_RectAlignHorizontal, (rect_val).horzAlign),                                                 \
			find_word(g_RectAlignVertical, (rect_val).vertAlign))

	#define WRITE_MENUDEF_FIELD_ACTION(str_key, str_action) \
		WRITE_MENU_FIELD_COND((str_action), MENUDEF_INDENT, "{ %s }", (str_key), action_optimize((str_action)))

	#define WRITE_MENUDEF_FIELD_MATERIAL(str_key, mtl) \
		WRITE_MENU_FIELD_COND((mtl), MENUDEF_INDENT, "\"%s\"", (str_key), *(mtl))

	#define WRITE_MENUDEF_FIELD_STRING(str_key, str_val) \
		WRITE_MENU_FIELD_COND((str_val), MENUDEF_INDENT, "\"%s\"", (str_key), (str_val))

	#define WRITE_MENUDEF_FIELD_KEYWORD(str_key, ptr_keyword, int_val) \
		WRITE_MENU_FIELD_COND((int_val), MENUDEF_INDENT, "%s", (str_key), find_word((ptr_keyword), (int_val)))

	#define WRITE_MENUDEF_FIELD_FLOAT(str_key, float_val) \
		WRITE_MENU_FIELD_COND((float_val) != 0, MENUDEF_INDENT, "%g", (str_key), (float_val))

	#define WRITE_MENUDEF_FIELD_INT(str_key, int_val) \
		WRITE_MENU_FIELD_COND((int_val), MENUDEF_INDENT, "%d", (str_key), (int_val))

/* ItemDef_t fields. */
	#define WRITE_ITEMDEF_FIELD_FLAG(str_key, int_condition) \
		WRITE_MENU_FIELD_COND((int_condition), ITEMDEF_INDENT, "", (str_key))

	#define WRITE_ITEMDEF_FIELD_EXPRESSION(str_key, expression)                                           \
		WRITE_MENU_FIELD_COND((expression).entries_count, ITEMDEF_INDENT, "%s ( %s );", "exp", (str_key), \
			generate_expression_string(&(expression)))
	...
	#define WRITE_ITEMDEF_FIELD_KEY(key_hndl)                                                          \
		WRITE_MENU_FIELD_COND((key_hndl), ITEMDEF_INDENT, "\"%c\" { %s }", "execKey", (key_hndl)->key, \
			action_optimize((key_hndl)->action))

	#define WRITE_ITEMDEF_FIELD_VEC4(str_key, vec4_val)                                                  \
		WRITE_MENU_FIELD_COND(1, ITEMDEF_INDENT, "%g %g %g %g", (str_key), (vec4_val)[0], (vec4_val)[1], \
			(vec4_val)[2], (vec4_val)[3])

	#define WRITE_ITEMDEF_FIELD_RECT(str_key, rect_val)                                                             \
		WRITE_MENU_FIELD_COND(((rect_val).x || (rect_val).y || (rect_val).w || (rect_val).h || (rect_val).horzAlign \
								  || (rect_val).vertAlign),                                                         \
			ITEMDEF_INDENT, "%g %g %g %g %s %s", (str_key), (rect_val).x, (rect_val).y, (rect_val).w, (rect_val).h, \
			find_word(g_RectAlignHorizontal, (rect_val).horzAlign),                                                 \
			find_word(g_RectAlignVertical, (rect_val).vertAlign))

	#define WRITE_ITEMDEF_FIELD_ACTION(str_key, str_action) \
		WRITE_MENU_FIELD_COND((str_action), ITEMDEF_INDENT, "{ %s }", (str_key), action_optimize((str_action)))

	#define WRITE_ITEMDEF_FIELD_MATERIAL(str_key, mtl) \
		WRITE_MENU_FIELD_COND((mtl), ITEMDEF_INDENT, "\"%s\"", (str_key), *(mtl))

	#define WRITE_ITEMDEF_FIELD_STRING(str_key, str_val) \
		WRITE_MENU_FIELD_COND((str_val), ITEMDEF_INDENT, "\"%s\"", (str_key), (str_val))

	#define WRITE_ITEMDEF_FIELD_KEYWORD(str_key, ptr_keyword, int_val) \
		WRITE_MENU_FIELD_COND((int_val), ITEMDEF_INDENT, "%s", (str_key), find_word((ptr_keyword), (int_val)))

	#define WRITE_ITEMDEF_FIELD_FLOAT(str_key, float_val) \
		WRITE_MENU_FIELD_COND((float_val) != 0, ITEMDEF_INDENT, "%g", (str_key), (float_val))

	#define WRITE_ITEMDEF_FIELD_INT(str_key, int_val) \
		WRITE_MENU_FIELD_COND((int_val), ITEMDEF_INDENT, "%d", (str_key), (int_val))

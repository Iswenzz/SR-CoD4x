...
typedef struct
{
	...
	void (*RunFrame)();
	bool (*SteamGetGroupMemberStatusByClientNum)(int clnum, uint64_t groupid, uint64_t reference,
		void (*callback)(int clientnum, uint64_t steamid, uint64_t groupid, uint64_t reference, bool m_bMember,
			bool m_bOfficer)); // reference is user configurable variable passed back to callback
	uint32_t (*GetAuthenticationTicket)(unsigned char* data, int* buflen,
		uint64_t* steamid);								 // To authenticate this server towards other servers

...
qboolean NET_TCPPacketEvent(netadr_t* from, byte* bufData, int len, int* connectionId,
	int* serviceId) // Returns qtrue if it wants to write data to socket
{
	int i, r;
	qboolean ret;
	msg_t msg;
...
void NET_TCPAddEventType(
	int (*tcpevent)(netadr_t* from, msg_t* msg, int connectionId), // returns -1 to close connection, 0 waiting for more
																   // data to read, 1 if it wants to write more data
	...
	void (*tcpconncloseevent)(netadr_t* from, int connectionId), int serviceId)
{
	...
	Com_Error(ERR_FATAL,
		"NET_TCPAddEventType: Out of redirect handles. Increase MAX_TCPEVENTS to add more redirect destinations");
}

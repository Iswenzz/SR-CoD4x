...
Returns the players Uid. Will only work with valid defined authserver or if another cod4-plugin is loaded with uid
support. If no UID is defined this function will return -1 Usage: int = self getUid();
*/
...
The userinfo will automatically update if myucvar changes onto the client computer. This can be used to transfer text
from client to server. Usage: string = self getUserinfo(userinfo key <string>);
*/
	...
	while (*countstring)
	{
		...
		if (halfPixelCounter >= maxHalfPixel)
		{
			...
			{ // we couln't find a space inside the upper half string length
				*countstring =
					0; // Mhh it is complicated to seperate the complete string here. We will just thrash one character
...
Returns an array of the string that got sperated in tokens.
It will count the number of characters of given string and will tokenize it so that it will never exceed the given
limit. This function tries to separate the string so that words remains complete Usage: array = StrTokByLen(string
...
/*
GScr_StrRepl
This functions finds in a given mainstring all occurrences of a given searchstring and replace those with a given
replacementstring This function returns the resulting string. The mainstring will stay unaffected.
...
void GScr_IsCvarDefined()
{
	...
	var_name = Scr_GetString(0);
	defined = Cvar_IsDefined(var_name); //@florczakraf Please pay attention about critical sections. I am not sure if
										// direct access is safe as you used it.
...
void PlayerCmd_IsStanding(scr_entref_t object)
{
	...
	Scr_AddBool(
		!(cl->lastUsercmd.buttons & KEY_MASK_CROUCH) && !(cl->lastUsercmd.buttons & KEY_MASK_PRONE) ? qtrue : qfalse);
}
...
static void PlayerCmd_GetSteamGroupMembershipCallback(int clientnum, uint64_t steamid, uint64_t groupid,
	uint64_t reference, bool m_bMember, bool m_bOfficer)
{
...
void PlayerCmd_GetSteamGroupMembership(scr_entref_t arg)
{
	...
	{
		Scr_Error(
			"Usage: self steamGroupMembershipQuery(<steamgroupid>, <::callbackfunction(<sid>, <gid>, <member>, "
			"<officer>)>)");
		return;
	}
	...
	qboolean status =
		SV_SApiGetGroupMemberStatusByClientNum(entityNum, gid, reference, PlayerCmd_GetSteamGroupMembershipCallback);
...
void Scr_PrecacheString_f()
{
	...
	if (level.initializing == qfalse)
		Scr_Error(
			"precacheString must be called before any wait statements "
 ...
 */
qboolean GScr_UpdateTagInternal2(gentity_t *ent, unsigned int tagName, cached_tag_mat_t *cachedTag,
...
void GScr_CloneBrushModelToScriptModel(scr_entref_t scriptModelEntNum)
{
	...
	if (brushEnt->classname != (unsigned short)scr_const.script_brushmodel
		&& brushEnt->classname != (unsigned short)scr_const.script_model
		&& brushEnt->classname != (unsigned short)scr_const.script_origin
		&& brushEnt->classname != (unsigned short)scr_const.light)
		Scr_ParamError(0,
			"brush model entity classname must be one of {script_brushmodel, script_model, script_origin, light}");
...
void PlayerCmd_SetStance(scr_entref_t playerEntNum)
{
	...
	if (stanceIdx != (unsigned short)scr_const.stand && stanceIdx != (unsigned short)scr_const.crouch
		&& stanceIdx != (unsigned short)scr_const.prone)
		Scr_ParamError(0, "stance must be one of {stand, crouch, prone}");

	BGEvent event = stanceIdx == (unsigned short)scr_const.stand ? EV_STANCE_FORCE_STAND
		: stanceIdx == (unsigned short)scr_const.crouch			 ? EV_STANCE_FORCE_CROUCH
...
void GScr_StrReplace()
{
	...
	while (mainstring[srcpos] != 0)
	{
		if (mainstring[srcpos] == '&' && mainstring[srcpos + 1] == '&' && mainstring[srcpos + 2] != 0)
		{
			...
			assemblypos += GScr_FetchAndReplaceInternal(argnum, assemblybuf + assemblypos,
				sizeof(assemblybuf) - (assemblypos + 1));
			srcpos += 3;
			continue;
		}
		// Escaping

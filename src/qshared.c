...
char *Com_ParseGetToken(char *line)
{
	...
	{
		if (parse_inquotes)
		{ // In case we are inside quotes step until the end quote forward
...
qboolean isNumeric(const char *string, int size)
{
	...
	if (size > 0)
	{ // If we have given a length compare the string limited by the length
	...
	{ // Search until the 1st space or null otherwise
...
void XML_CloseTag(xml_t *base)
{
	...
	if (base->parents == 0)
	{
		Com_PrintError(CON_CHANNEL_SYSTEM,
			"XML_CloseTag: Invalid close attempt in XML. Attempt to close more elements than you have opened.\n");
	...
	for (i = 0; *root != ' ' && *root != 0 && *root != '>' && i < sizeof(buffer); stringptr++, root++, i++)
		*stringptr = *root;
	*stringptr = 0;
	if (base->last)
	{
...
/*
int __cdecl KeyValueToField(char *pStruct, cspField_t *pField, const char *pszKeyValue, const int iMaxFieldTypes, int
(__cdecl *parseSpecialFieldType)(char *, const char *, const int, const int), void (__cdecl *parseStrcpy)(char *, const
char *))
{
  char name;
  char dest;
...
*/
int __cdecl KeyValueToField(char *pStruct, cspField_t *pField, const char *pszKeyValue, const int iMaxFieldTypes,
	int(__cdecl *parseSpecialFieldType)(char *, const char *, const int, const int),
	void(__cdecl *parseStrcpy)(char *, const char *))
{
	char dest[0x2000];
...
bool __cdecl ParseConfigStringToStruct(char *pStruct, cspField_t *pFieldList, const int iNumFields,
	const char *pszBuffer, const int iMaxFieldTypes,
	int(__cdecl *parseSpecialFieldType)(char *, const char *, const int, const int),
	void(__cdecl *parseStrCpy)(char *, const char *))
{
	char *pszKeyValue;
	...
	while (iField < iNumFields)
	{
		...
		{
			error |=
				KeyValueToField(pStruct, pField, pszKeyValue, iMaxFieldTypes, parseSpecialFieldType, parseStrCpy) == 0;
		}
		++iField;
		++pField;
	}

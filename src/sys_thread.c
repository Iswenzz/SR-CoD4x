...
void Sys_RunThreadCallbacks()
{
	...
	for (tcb = thread_callbacks, i = 0; i < MAX_CALLBACKS; i++)
	{
		...
		if (thread_callbacks[i].callbackMain != NULL)
			thread_callbacks[i].callbackMain(tcb->callback_args[0], tcb->callback_args[1], tcb->callback_args[2],
...
qboolean __cdecl Sys_SpawnDatabaseThread(void (*db_proc)(unsigned int p))
{
	...
	threadHandle[THREAD_CONTEXT_DATABASE] =
		Sys_CreateThreadWithHandle(Sys_ThreadMain, &threadId[THREAD_CONTEXT_DATABASE], (void*)THREAD_CONTEXT_DATABASE);

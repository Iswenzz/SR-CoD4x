...
qboolean Sec_SignHash(const char *in, size_t inSize, rsa_key *key, char *out, size_t *outSize)
{
	...
	int res = rsa_sign_hash_ex((const unsigned char *)in, is, (unsigned char *)out, &os, LTC_LTC_PKCS_1_V1_5, NULL, 0,
		0, 16, key);
	*outSize = os;
	return res;
}
...
{
	int n = 0;
	int res = rsa_verify_hash_ex((const unsigned char *)sig, sigSize, (const unsigned char *)hash, hashSize,
...
qboolean Sec_VerifyMemory(const char *expectedb64hash, void *memory, int lenmem)
{
	...
	if (base64_decode((byte *)expectedb64hash, strlen((char *)expectedb64hash), ciphertext,
			(long unsigned int *)&ciphertextlen)
		!= CRYPT_OK)
	{

...
const BotAction_t BotActions[] = { { "gostand", KEY_MASK_JUMP }, { "gocrouch", KEY_MASK_CROUCH },
	{ "goprone", KEY_MASK_PRONE }, { "fire", KEY_MASK_FIRE }, { "melee", KEY_MASK_MELEE }, { "frag", KEY_MASK_FRAG },
	{ "smoke", KEY_MASK_SMOKE }, { "reload", KEY_MASK_RELOAD }, { "sprint", KEY_MASK_SPRINT },
	{ "leanleft", KEY_MASK_LEANLEFT }, { "leanright", KEY_MASK_LEANRIGHT }, { "ads", KEY_MASK_ADS_MODE | KEY_MASK_ADS },
...
static void Bot_CalculateRotationForOrigin(gentity_t *bot, vec3_t origin, float duration)
{
	...
	for (i = 0; i < 2; ++i)
	{
		...
		g_botai[bot_num].rotFrac[i] = (short int)(((rot[i] / 360.0 * 65535) / g_botai[bot->s.clientNum].rotIterCount));
	}
...
static void scr_botlookatplayer(scr_entref_t ent_num)
{
	...
	if (!GScr_UpdateTagInternal2(target, tag_name, &level.cachedTagMat, qtrue))
		Scr_ParamError(1,
			va("tag '%s' does not exist in model '%s' "
			   "(or any attached submodels)",
				SL_ConvertToString(tag_name), SL_ConvertToString(SV_GetModelConfigstringIndex(bot->model))));

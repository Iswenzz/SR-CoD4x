...
void Cvar_ValueToStr(cvar_t const* cvar, char* bufvalue, int sizevalue, char* bufreset, int sizereset, char* buflatch,
	int sizelatch);
...
*/
static cvar_t* Cvar_Register(const char* var_name, cvarType_t type, unsigned short flags, CvarValue value,
	CvarLimits limits, const char* description)
{
	...
	else if (var)
	{
		...
		*/
		...
		{
			Com_Error(ERR_FATAL,
				"Cvar_Register: Tried to reregister an already registered Cvar \'%s\' as a different type\n", var_name);
			Sys_LeaveCriticalSection(CRITSECT_CVAR);
			return NULL;
		}
...
void Cvar_SetWithType(const char* var_name, CvarValue value, cvarType_t type, qboolean force)
{
	...
	if (var)
	{
		...
		{
			Com_Error(ERR_FATAL, "Cvar_SetWithType: Existing Cvar %s has a different type. Old type: %d New type: %d",
	...
	int sizelatch)
{
	...
	case CVAR_VEC2:
		...
		if (bufreset)
			Com_sprintf(bufreset, sizereset, "%g %g", cvar->resetVec2[0], cvar->resetVec2[1]);
		if (buflatch)
			Com_sprintf(buflatch, sizelatch, "%g %g", cvar->latchedVec2[0], cvar->latchedVec2[1]);
		return;
	case CVAR_VEC3:
		if (bufvalue)
			Com_sprintf(bufvalue, sizevalue, "%g %g %g", cvar->vec3[0], cvar->vec3[1], cvar->vec3[2]);
		if (bufreset)
			Com_sprintf(bufreset, sizereset, "%g %g %g", cvar->resetVec3[0], cvar->resetVec3[1], cvar->resetVec3[2]);
		if (buflatch)
			Com_sprintf(buflatch, sizelatch, "%g %g %g", cvar->latchedVec3[0], cvar->latchedVec3[1],
				cvar->latchedVec3[2]);
		return;
	case CVAR_VEC4:
		if (bufvalue)
			Com_sprintf(bufvalue, sizevalue, "%g %g %g %g", cvar->vec4[0], cvar->vec4[1], cvar->vec4[2], cvar->vec4[3]);
		if (bufreset)
			Com_sprintf(bufreset, sizereset, "%g %g %g %g", cvar->resetVec4[0], cvar->resetVec4[1], cvar->resetVec4[2],
				cvar->resetVec4[3]);
		if (buflatch)
			Com_sprintf(buflatch, sizelatch, "%g %g %g %g", cvar->latchedVec4[0], cvar->latchedVec4[1],
				cvar->latchedVec4[2], cvar->latchedVec4[3]);
		return;
	case CVAR_COLOR:
		if (bufvalue)
			Com_sprintf(bufvalue, sizevalue, "%.3g %.3g %.3g %.3g", (float)cvar->color.rgba[0] / (float)0xff,
				(float)cvar->color.rgba[1] / (float)0xff, (float)cvar->color.rgba[2] / (float)0xff,
				(float)cvar->color.rgba[3] / (float)0xff);
		if (bufreset)
			Com_sprintf(bufreset, sizereset, "%.3g %.3g %.3g %.3g", (float)cvar->resetColor.rgba[0] / (float)0xff,
				(float)cvar->resetColor.rgba[1] / (float)0xff, (float)cvar->resetColor.rgba[2] / (float)0xff,
				(float)cvar->resetColor.rgba[3] / (float)0xff);
		if (buflatch)
			Com_sprintf(buflatch, sizelatch, "%.3g %.3g %.3g %.3g", (float)cvar->latchedColor.rgba[0] / (float)0xff,
				(float)cvar->latchedColor.rgba[1] / (float)0xff, (float)cvar->latchedColor.rgba[2] / (float)0xff,
				(float)cvar->latchedColor.rgba[3] / (float)0xff);
		return;
	case CVAR_INT:
		if (bufvalue)
...
qboolean Cvar_Command(void)
{
	...
	if (Cmd_Argc() == 1)
	{
		...
		if (v->description != NULL)
			Com_Printf(CON_CHANNEL_CVAR,
				"\"%s\" is: \"%s" S_COLOR_WHITE "\" default: \"%s" S_COLOR_WHITE "\" info: \"%s" S_COLOR_WHITE "\"\n",
...
cvar_t* Cvar_RegisterFloat(const char* name, float val, float min, float max, unsigned short flags,
...
cvar_t* Cvar_RegisterVec2(const char* name, float x, float y, float min, float max, unsigned short flags,
	const char* description)
{
	cvar_t* cvar;
	CvarLimits limits;
...
cvar_t* Cvar_RegisterVec3(const char* name, float x, float y, float z, float min, float max, unsigned short flags,
	const char* description)
{
	cvar_t* cvar;
	CvarLimits limits;
...
cvar_t* Cvar_RegisterVec4(const char* name, float x, float y, float z, float imag, float min, float max,
...
cvar_t* Cvar_RegisterColor(const char* name, float r, float g, float b, float alpha, unsigned short flags,
	const char* description)
{
	cvar_t* cvar;
	CvarLimits limits;
...
cvar_t* Cvar_RegisterEnum(const char* name, const char** strings, int integer, unsigned short flags,
	const char* description)
{
	cvar_t* cvar;
	CvarLimits limits;

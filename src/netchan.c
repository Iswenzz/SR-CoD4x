...
*/
void Netchan_Setup(netsrc_t sock, netchan_t *chan, netadr_t adr, unsigned int qport, byte *unsentBuffer,
	int unsentBufferSize, byte *fragmentBuffer, int fragmentBufferSize)
{
	...
	if (showpackets->boolean)
	{
		if (fragmented)
		{
			Com_Printf(CON_CHANNEL_NETWORK, "%s recv %4i : s=%i fragment=%i,%i\n", netsrcString[chan->sock],
				msg->cursize, sequence, fragmentStart, fragmentLength);
		}
		else
		{
		...
		if (fragmentStart != chan->fragmentLength)
		{
			if (showdrop->boolean || showpackets->boolean)
			{
				Com_Printf(CON_CHANNEL_NETWORK,
					"%s:Dropped a message fragment seq: %d, fragmentStart: %d, chan->fragmentLength: %d\n",
			...
			|| chan->fragmentLength + fragmentLength > chan->fragmentBufferSize)
		{
			if (showdrop->boolean || showpackets->boolean)
			{
				Com_Printf(CON_CHANNEL_NETWORK, "%s:illegal fragment length: Current %i Fragment length %i Max %i\n",
					NET_AdrToString(&chan->remoteAddress), chan->fragmentLength, fragmentLength,
					chan->fragmentBufferSize);
			}
			return qfalse;
		}
...
qboolean NET_SendPacket(netsrc_t sock, int length, const void *data, netadr_t *to)
{
	...
	if (showpackets->boolean && *(int *)data == -1)
	{
		...
		Com_Printf(CON_CHANNEL_NETWORK, "NET_SendPacket %4i destination %s\n", length,
			NET_AdrToStringMT(to, string, sizeof(string)));
	}

...
typedef struct
{ // It is only for timelimited tempbans to prevent happy reconnecting and sitting in server for 2 minutes until player
  // is detected or keep to advertise with playernick while reconnecting
	netadr_t remote;
	char banmsg[256];
...
const char* SV_WriteBanTimelimit(int timeleftsecs, char* outbuffer, int outbufferlen)
{
	...
	{
		int remaining = (int)(timeleftsecs
			+ 60); // in seconds (+60 for fixing up a display error when only some seconds are remaining)
		...
		Com_sprintf(outbuffer, outbufferlen,
			"This is a temporary ban and will expire in\n %i days %i hours %i minutes\n", d, h, m);
		return outbuffer;
	}
}

char* SV_PlayerBannedByip(netadr_t* netadr, char* message, int len)
{ // Gets called in SV_DirectConnect
	ipBanList_t* this;
	int i;
	char outbuffer[512];
	int timeleftsecs;
		...
		if (NET_CompareBaseAdr(netadr, &this->remote))
		{
			...
			if (timeleftsecs > 1)
			{
				Com_sprintf(message, len, "%s\n%s\n", this->banmsg,
					SV_WriteBanTimelimit(timeleftsecs, outbuffer, sizeof(outbuffer)));
				return message;
			}
		}
	}
...
{ // Gets called by future implemented ban-commands and if a prior ban got enforced again
	...
	for (list = &ipBans[0], i = 0; i < MAX_IPBANS; list++, i++)
	{ // At first check whether we have already an entry for this player
	...
	if (duration > ipbantime->integer * 60 || expire == -1)
		duration = ipbantime->integer
			* 60; // Don't ban IPs for more than MAX_IPBAN_MINUTES minutes as they can be shared (Carrier-grade NAT)
...
void SV_InitBanlist()
{
	ipbantime = Cvar_RegisterInt("banlist_maxipbantime", MAX_DEFAULT_IPBAN_MINUTES, 0, 20160, 0,
	...
	sv_banappealminhours = Cvar_RegisterInt("banlist_appealminhours", DEFAULT_APPEAL_MINHOURS, 0, 336, 0,

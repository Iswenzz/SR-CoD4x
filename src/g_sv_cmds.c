...
void Init_CallVote(void)
{
	...
	g_voteBanTime =
		Cvar_RegisterInt("g_voteBanTime", 15, 1, 240, 0, "Duration a player is banned after successful votekick");
	g_voteMaxVotes = Cvar_RegisterInt("g_voteMaxVotes", 2, 1, 10, 0, "How many votes a player can call");
	g_voteVoteGametypes = Cvar_RegisterString("g_voteVoteGametypes", "", 0,
		"Contains a list of gametypes that are allowed to vote. Empty list = all");
	g_voteKickMinPlayers = Cvar_RegisterInt("g_voteKickMinPlayers", 5, 0, 14, 0,
		"How many active players are needed on server to allow calling a kickvote");
	...
	g_voteAllowMap = Cvar_RegisterInt("g_voteAllowMap", 1, 0, 2, 0,
		"Allow calling next map setting votes - 0=disabled, 1=only from rotation, 2=Any map");
...
void Cmd_CallVote_f(gentity_t *ent)
{
	...
	else
	{
		...
		SV_GameSendServerCommand(ent - g_entities, 0,
			va("%c \"GAME_VOTECOMMANDSARE\x15 map_restart, map_rotate, map <mapname>, g_gametype <gametype>, kick "
			   "<player or clientnum>, typemap <gametype> <map>\"\0",
				0x65));
		return;
	}
	...
	if (!Q_stricmp(arg1, "typemap"))
	{
		if (*g_voteVoteGametypes->string)
		{
			...
			{
				SV_GameSendServerCommand(ent - g_entities, 0,
					va("%c \"Voting for gametype %s is disabled on this server\"\0", 0x65, arg2));
				return;
			}
		}
		...
		if (!(g_voteFlags & VOTEFLAGS_ANYMAP))
		{
			...
			{
				SV_GameSendServerCommand(ent - g_entities, 0,
					va("%c \"Voting for map %s is disabled on this server\"\0", 0x65, arg3));
				return;
			}
		}
		...
		Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
			"Set next map to: %s and gametype to: \x14%s", arg3, arg2 /*g_gametypes->gametype[i].gametypereadable*/);
	}
	else if (!Q_stricmp(arg1, "g_gametype"))
	{
		if (*g_voteVoteGametypes->string)
		...
		Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString), "Set gametype to: \x15%s and restart",
			arg2 /*g_gametypes->gametype[i].gametypename*/);
	}
	...
	else if (!Q_stricmp(arg1, "map"))
	{
		if (!(g_voteFlags & VOTEFLAGS_ANYMAP))
		{
			...
			{
				SV_GameSendServerCommand(ent - g_entities, 0,
					va("%c \"This server does not allow voting for maps which aren't part of map-rotation\"\0", 0x65));
				return;
			}
		}
		...
		if (*g_votedMapName->string)
		{
			if (*g_votedGametype->string)
				Com_sprintf(level.voteString, sizeof(level.voteString),
					"set g_gametype %s; map %s; set g_votedGametype \"\"; set g_votedMapName \"\"\n",
	...
	else if (!Q_stricmp(arg1, "tempbanuser") || !Q_stricmp(arg1, "kick"))
	{
		...
		{ // found a client # to kick, so override votestring with better one
			...
			Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString), "%s wants to kick and tempban: %s",
				ent->client->sess.newnetname, level.clients[kicknum].sess.newnetname);

			if (svs.clients[kicknum].power > 5)
			{
				SV_GameSendServerCommand(ent - g_entities, 0,
					va("%c \"Can not call a vote against a protected player\"\0", 0x65));
...
void G_SayTo(gentity_t *ent, gentity_t *other, int mode, int color, const char *teamname, const char *name,
	const char *message)
{
	char buf[64];
	char v16[128];
	...
	if (ent->client->sess.sessionState != SESS_STATE_PLAYING && other->client->sess.sessionState == SESS_STATE_PLAYING
		&& !g_deadChat->boolean)
	{
		return;
	}
	...
	SV_GameSendServerCommand(other->s.number, 0,
		va("%c \"\x15%s%c%c%s\"", 'i' - (mode != 1), v16, '^', color, message));
}
...
void G_AddChatRedirect(void (*rd_dest)(const char *, int, int))
{
	...
	Com_Error(ERR_FATAL,
		"G_AddChatRedirect: Out of redirect handles. Increase MAX_REDIRECTDESTINATIONS to add more redirect "
		"destinations");
}

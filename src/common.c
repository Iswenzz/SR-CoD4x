...
static void Com_InitCvars(void)
{
	...
	com_timescale =
		Cvar_RegisterFloat("timescale", 1.0, 0.0, 1000.0, CVAR_CHEAT | CVAR_SYSTEMINFO, "Scale time of each frame");
	com_fixedtime = Cvar_RegisterInt("fixedtime", 0, 0, 1000, 0x80, "Use a fixed time rate for each frame");
	com_maxFrameTime = Cvar_RegisterInt("com_maxFrameTime", 100, 50, 1000, 0,
	...
	com_shortversion =
		Cvar_RegisterString("shortversion", Q3_VERSION, CVAR_ROM | CVAR_SERVERINFO, "Short game version");

	Cvar_RegisterInt("build", Sys_GetBuild(), Sys_GetBuild(), Sys_GetBuild(), CVAR_ROM | CVAR_SERVERINFO,
		"Count of SCM commits since project has been started");
	...
	com_securemodevar = Cvar_RegisterBool("securemode", qfalse, CVAR_INIT,
		"CoD4 runs in secure mode which restricts execution of external scripts/programs and loading of unauthorized "
		"shared libraries/plugins. This is recommended in a shared hosting environment");
...
void Com_Init(char* commandLine)
{
	...
	Com_Printf(CON_CHANNEL_SYSTEM, "%s %s %s build %i %s\n", GAME_STRING, Q3_VERSION, PLATFORM_STRING, Sys_GetBuild(),
		__DATE__);
...
qboolean Com_TryDownloadAndExecGlobalConfig()
{
	...
	curfileobj = HTTPRequest("https://raw.githubusercontent.com/callofduty4x/CoD4x_Server/master/globalconfig.cfg",
		"GET", NULL, "Accept: text/plain; charset=utf-8\r\n");
	...
	Q_strncpyz(content, (const char*)curfileobj->recvmsg.data + curfileobj->headerLength,
		curfileobj->contentLength + 1);

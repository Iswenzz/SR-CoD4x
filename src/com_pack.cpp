...
extern "C"
{
	...
	void __cdecl Vec2UnpackTexCoords(PackedTexCoords in, float *out)
	{
		...
		if (in.packed >> 16)
		{
			v3.dw = ((in.packed >> 16 << 16) & 0x80000000)
				| (((((in.packed >> 16 << 14) & 0xFFFC000) - (~(in.packed >> 16 << 14) & 0x10000000)) ^ 0x80000000)
					>> 1);
		}
		else
		{
		...
		*out = v3.f;
		if (in.w)
		{
			v2.dw = ((in.w << 16) & 0x80000000)
				| (((((in.w << 14) & 0xFFFC000) - (~(in.w << 14) & 0x10000000)) ^ 0x80000000) >> 1);
		}
		else
		{
			v2.f = 0.0;
	...
	PackedTexCoords __cdecl Vec2PackTexCoords(const float *in)
	{
		...
		ptc.packed =
			(((v2 & 0x3FFF) | ((_in[1].sdw >> 16) & 0xC000)) + (((v3 & 0x3FFF) | ((_in[0].dw >> 16) & 0xC000)) << 16));
		return ptc;
	}
}

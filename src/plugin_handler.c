...
char PHandler_Events[PLUGINS_ITEMCOUNT][32] = {
	...
	"OnInfoRequest", "OnPlayerDC", "OnPlayerConnect", "OnExitLevel", "OnMessageSent", "OnFrame", "OnOneSecond",
	"OnTenSeconds", "OnClientAuthorized", "OnClientSpawn", "OnClientEnterWorld", "OnTcpServerPacket", "OnUdpNetEvent",
	"OnUdpNetSend", "OnSpawnServer", "OnPreFastRestart", "OnPostFastRestart", "OnPreGameRestart", "OnPostGameRestart",
	"OnClientUserinfoChanged", "OnClientCommand", "OnClientMoveCommand", "OnPlayerKilled", "OnPlayerWantReservedSlot",
	"OnFilesystemStarted", "OnPlayerGotAuthInfo", "OnPlayerAddBan", "OnPlayerGetBanStatus", "OnPlayerRemoveBan",
	"OnModuleLoaded", "OnScreenshotArrived", "OnTerminate"
};
...
qboolean PHandler_VerifyPlugin(void *buf, int len)
{
	...
	for (i = 0, curstring = strings[i]; curstring != NULL; i++, curstring = strings[i])
	{
		...
		if(strcmp(curstring,"malloc")==0 || strcmp(curstring,"calloc")==0 || strcmp(curstring,"realloc")==0 ||
			   ...
			   strcmp(curstring,"asprintf")==0 || strcmp(curstring,"free")==0){ // malloc, calloc, realloc, free,
		printf, , asprintf, scanf Com_Printf(CON_CHANNEL_PLUGINS,"The plugin file contains one of the disallowed
		functions! Disallowed function name: \"%s\".\nPlease refer to the documentation for details.\nPlugin load
...
void PHandler_Load(char *name) // Load a plugin, safe for use
{
	...
	if (com_securemode)
	{
		...
		if (Q_stricmp(hash, PLUGINGAMERANGER_HASH) && Q_stricmp(hash, PLUGINCENSOR_HASH)
			&& Q_stricmp(hash, PLUGINANTISPAM_HASH))
		{
			Com_Printf(CON_CHANNEL_PLUGINS, "Tiger = %s\n", hash);
			Com_PrintError(CON_CHANNEL_PLUGINS,
				"%s checksum missmatch! This plugin will not be loaded in securemode.\n", realpath);
			return;
		}
	}
	...
	if (pluginFunctions.plugins[i].OnInit == NULL)
	{
		...
		memset(pluginFunctions.plugins + i, 0x00,
			sizeof(plugin_t)); // We need to remove all references so we can dlclose.
	...
	if (pluginFunctions.plugins[i].OnInfoRequest)
	{
		...
		if (info.handlerVersion.major != PLUGIN_HANDLER_VERSION_MAJOR
			|| (info.handlerVersion.minor - info.handlerVersion.minor % 100)
				!= (PLUGIN_HANDLER_VERSION_MINOR - PLUGIN_HANDLER_VERSION_MINOR % 100))
		{
			Com_PrintError(CON_CHANNEL_PLUGINS,
				"This plugin might not be compatible with this server version! Requested plugin handler version: "
				"%d.%d, server's plugin handler version: %d.%d. Unloading the plugin...\n",
				info.handlerVersion.major, info.handlerVersion.minor, PLUGIN_HANDLER_VERSION_MAJOR,

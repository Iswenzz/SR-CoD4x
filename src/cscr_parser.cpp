...
extern "C" void __regparm3 Scr_AddSourceBufferInternal(const char *extFilename, const char *codePos, char *sourceBuf,
	int len, bool doEolFixup, bool archive);
...
void Scr_PrintFunctionPosSpreadSheet(conChannel_t channel, const char *filename, const char *buf,
	unsigned int sourcePos)
{
	const char *s;
	unsigned int lineNum;
...
const char *Scr_PrevCodePosFunctionName(const char *codePos)
{
	...
	Scr_GetFunctionLineNumInternal(gScrParserPub.sourceBufferLookup[bufferIndex].sourceBuf,
		Scr_GetPrevSourcePos(codePos - 1, 0), &startLine);
	return startLine;
}
...
void Scr_PrintPrevCodePosSpreadSheet(conChannel_t channel, const char *codePos, bool summary, bool functionSummary)
{
	...
	if (codePos)
	{
		...
		else if (gScrVarPub.programBuffer && Scr_IsInOpcodeMemory(codePos))
		{
			...
			{
				Scr_PrintFunctionPosSpreadSheet(channel, gScrParserPub.sourceBufferLookup[bufferIndex].buf,
					gScrParserPub.sourceBufferLookup[bufferIndex].sourceBuf, Scr_GetPrevSourcePos(codePos - 1, 0));
			}
			else
			{
				Scr_PrintSourcePosSpreadSheet(channel, gScrParserPub.sourceBufferLookup[bufferIndex].buf,
					gScrParserPub.sourceBufferLookup[bufferIndex].sourceBuf, Scr_GetPrevSourcePos(codePos - 1, 0));
			}
		}
		else
...
extern "C"
{
	...
	unsigned int __cdecl Scr_GetSourceBuffer(const char *codePos)
	{
		...
		for (bufferIndex = gScrParserPub.sourceBufferLookupLen - 1; bufferIndex
			 && (!gScrParserPub.sourceBufferLookup[bufferIndex].codePos
				 || gScrParserPub.sourceBufferLookup[bufferIndex].codePos > codePos);
			 --bufferIndex)
			;
		return bufferIndex;
	}
	...
	void __cdecl Scr_PrintPrevCodePos(conChannel_t channel, const char *codePos, unsigned int index)
	{
		...
		else
		{
			if (gScrVarPub.developer)
			{
				...
				{
					bufferIndex = Scr_GetSourceBuffer(codePos - 1);
					Scr_PrintSourcePos(channel, gScrParserPub.sourceBufferLookup[bufferIndex].buf,
						gScrParserPub.sourceBufferLookup[bufferIndex].sourceBuf,
	...
	char *__cdecl Scr_AddSourceBuffer(const char *filename, const char *extFilename, const char *codePos, bool archive)
	{
		...
		if (len >= 0)
		{
			...
			*dest = 0;
			...
			{
				Hunk_FreeDebugMem(
					gScrParserGlob.saveSourceBufferLookup[gScrParserGlob.saveSourceBufferLookupLen].sourceBuf);
			}
		}
		else
		{
...
void __cdecl Scr_GetTextSourcePos(const char *buf, const char *codePos, char *line)
{
	...
	if (gScrVarPub.developer && codePos && codePos != &g_EndPos && gScrVarPub.programBuffer
		&& Scr_IsInOpcodeMemory(codePos))
	{
		bufferIndex = Scr_GetSourceBuffer(codePos - 1);
		Scr_GetLineInfo(gScrParserPub.sourceBufferLookup[bufferIndex].sourceBuf, Scr_GetPrevSourcePos(codePos - 1, 0),
			&col, line);
	}
	else
	{

...
void *PE32_FindFileOffset(void *buff, int len, DWORD RVA)
{
	...
	{
		IMAGE_SECTION_HEADER sectionHeader = *(IMAGE_SECTION_HEADER *)((DWORD)buff + dosHeader->e_lfanew
		...
		{
			/* Out of range */
			if (RVA - dwSectionRVA + dwSectionRawBase < dosHeader->e_lfanew + sizeof(IMAGE_NT_HEADERS)
				|| RVA - dwSectionRVA + dwSectionRawBase > len)
			{
				return NULL;
			}
			/* Valid real offset */
...
char **PE32_GetStrTable(void *buff, int len, sharedlib_data_t *text)
{
	...
	*/
	...
	if ((DWORD)(&(ntHeader->OptionalHeader)) < (DWORD)buff
		|| (DWORD)(&(ntHeader->OptionalHeader)) + sizeof(IMAGE_OPTIONAL_HEADER) > len + (DWORD)buff)
	{
		Com_Printf(CON_CHANNEL_SYSTEM, "Not a valid PE file\n");
		return NULL;
	}
	...
	if ((DWORD)(&OptionalHeader->DataDirectory) < (DWORD)buff
		|| (DWORD)(&OptionalHeader->DataDirectory) + 15 * sizeof(IMAGE_DATA_DIRECTORY) > len + (DWORD)buff)
	{
		Com_Printf(CON_CHANNEL_SYSTEM, "Not a valid PE file\n");
		return NULL;
	}

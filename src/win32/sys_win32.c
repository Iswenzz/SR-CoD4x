...
void Sys_LoadLibraryError(char *errormessage, int maxlen)
{
	...
	if (lastError != 0)
	{
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, lastError, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
			(LPSTR)errormessage, maxlen - 1, NULL);
	}
	else
	{
...
HANDLE Sys_CreateThreadWithHandle(void *(*ThreadMain)(void *), threadid_t *tid, void *arg)
{
	...
	if (thid == NULL)
	{
		...
		{
			FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, lastError, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
				(LPSTR)errMessageBuf, sizeof(errMessageBuf) - 1, NULL);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (copylen >= (sizeof(lpFilename) - 1))
	{
		...
		Sys_SetBinaryPath("");
		fprintf(stderr,
			"Path is too long. The whole path to location of this .exe file must not exceed 254 characters");
		return 1;
	}
	else
	{
...
HANDLE __cdecl Sys_CreateEvent(qboolean bManualReset, qboolean bInitialState, const char *name)
{
	...
	return CreateEventA(&sa, bManualReset, bInitialState,
		NULL); // Name must be NULL or it will interact with other processes
}
...
int Sys_ReadCertificate(void *cacert,

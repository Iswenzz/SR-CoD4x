...
tcpclientstate_t NET_TCPAuthPacketEvent(netadr_t *remote, byte *bufData, int cursize, int *sock, int *connectionId,
	int *serviceId)
{
...
const char *NET_AdrMaskToStringInternal(netadr_t *a, char *s, int len)
{
	...
	{
		Com_sprintf(s, len, "%i.%i.x.x:%hu", a->ip[0], a->ip[1], BigShort(a->port));
	}
	else if (a->type == NA_IP6 || a->type == NA_TCP6)
	{
		Com_sprintf(s, len, "[%02x%02x:%02x%02x:%02x%02x:x:x:x:x:x]:%hu", a->ip6[0], a->ip6[1], a->ip6[2], a->ip6[3],
			a->ip6[4], a->ip6[5], BigShort(a->port));
	}
	return s;
}
...
const char *NET_AdrToConnectionStringCommon(netadr_t *a, const char *(NET_AdrToString_a)(netadr_t *),
	...
	if (socket != INVALID_SOCKET)
	{
		...
		else
		{
			/*			memset( ((struct sockaddr_in *)&from)->sin_zero, 0, 8 );
			   ...
			   net_message->data[2] != 0 || net_message->data[3] != 1 ) { return qfalse;
							}
							net_from->type = NA_IP;
...
qboolean Sys_SendPacket(int length, const void *data, netadr_t *to)
{
	...
	if ((to->type == NA_IP && to->sock == INVALID_SOCKET) || (to->type == NA_IP6 && to->sock == INVALID_SOCKET)
		...
		if (ret == SOCKET_ERROR)
		{
			...
			if (err2 != EAGAIN)
			{
				...
				Com_PrintError(CON_CHANNEL_NETWORK, "Sys_SendPacket: socket %d Conn: %s ==> %s failed with: %s\n",
					to->sock, tmpstr, NET_AdrToStringMT(to, adrstr, sizeof(adrstr)),
	...
	*err = 0;
	...
	if (newsocket < 0 || newsocket >= MAX_SOCKETLIMIT)
	{
		Com_Printf(CON_CHANNEL_NETWORK,
			"WARNING: NET_IP4Socket: socket is out of range 0 to 1023. Are there too many open connections or "
			"files?\n");
	...
	else
	{
		/*
			// Short WAIT_STATE time
				...
				if( setsockopt( newsocket, SOL_SOCKET, SO_LINGER, (char *) &so_linger, sizeof(so_linger) ) ==
		   SOCKET_ERROR ) { Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_IP4Socket: setsockopt SO_LINGER: %s\n",
		   NET_ErrorStringMT(errstr, sizeof(errstr)) );
				}
		*/
		reuse = 1;
...
int NET_IP6Socket(char *net_interface, int port, struct sockaddr_in6 *bindto, int *err, qboolean tcp)
{
	...
	*err = 0;
	...
	{
		Com_Printf(CON_CHANNEL_NETWORK,
			"WARNING: NET_IP6Socket: socket is out of range 0 to 1023. Are there too many open connections or "
			"files?\n");
		closesocket(newsocket);
		return INVALID_SOCKET;
	}
	...
	if (tcp)
	{
		/*
			// Short WAIT_STATE time
				...
				if( setsockopt( newsocket, SOL_SOCKET, SO_LINGER, (char *) &so_linger, sizeof(so_linger) ) ==
		   SOCKET_ERROR ) { Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_IP6Socket: setsockopt SO_LINGER: %s\n",
		   NET_ErrorStringMT(errstr, sizeof(errstr)) );
				}
		*/
...
int NET_IPSocket(char *net_interface, int port, int *err, qboolean tcp)
{
	...
	*err = 0;
	...
	{
		Com_Printf(CON_CHANNEL_NETWORK,
			"WARNING: NET_IPSocket: socket is out of range 0 to 1023. Are there too many open connections or files?\n");
		closesocket(newsocket);
		return INVALID_SOCKET;
	}
#endif
...
void NET_SetMulticast6(void)
{
	...
	if(!*net_mcast6addr->string || !Sys_StringToSockaddr(net_mcast6addr->string, (struct sockaddr *) &addr,
...
one. multicast6_socket = ip6_socket;
	}
	else
	{
		if((multicast6_socket = NET_IP6Socket(net_mcast6addr->string, ntohs(boundto.sin6_port), NULL, &err, qfalse)) ==
INVALID_SOCKET)
		{
			// If the OS does not support binding to multicast addresses, like WinXP, at least try with the normal file
descriptor. multicast6_socket = ip6_socket;
		}
	}
...
// Autodetection of internet version 6 addresses which do not change. Required as players should learn only permanent
// server addresses and not storing temporarly addresses in favorites
qboolean IsStaticIP6Addr(nip_localaddr_t *localaddr)
{
	...
	// Last: On Windows there exist non EUI-64 permanent IPv6 addresses but link local address interface identifier and
	// public ip interface identifier match
	for (z = 0; z < MAX_IPS; ++z)
	{
		...
		if (probebaddr6[0] == 0xfe && probebaddr6[1] == 0x80 && probebaddr6[2] == 0x00 && probebaddr6[3] == 0x00
			&& probebaddr6[4] == 0x00 && probebaddr6[5] == 0x00 && probebaddr6[6] == 0x00 && probebaddr6[7] == 0x00)
		{
			// Link local address
...
static void NET_GetLocalAddress(void)
{
	...
	if (!getaddrinfo(hostname, NULL, &hint, &res))
	{
		...
		/* Windows doesn't seem to add the loopback interface to its list of available interfaces. We have to assume
		 * they are there. */
		if (has_ip4 && !has_loopback4
			&& Sys_StringToSockaddr("localhost", (struct sockaddr *)&localhostadr, sizeof(localhostadr), AF_INET))
		{
			NET_AddLocalAddress("localhost", (struct sockaddr *)&localhostadr, (struct sockaddr *)&mask4);
		}

		if (has_ip6 && !has_loopback6
			&& Sys_StringToSockaddr("localhost", (struct sockaddr *)&localhostadr, sizeof(localhostadr), AF_INET6))
		{
...
static qboolean NET_GetCvars(void)
{
	...
	net_mcast6addr = Cvar_RegisterString("net_mcast6addr", NET_MULTICAST_IP6, CVAR_LATCH | CVAR_ARCHIVE,
		"IPv6 Network multicast address");
	modified += net_mcast6addr->modified;
	net_mcast6addr->modified = qfalse;

#ifdef _WIN32
	net_mcast6iface =
		Cvar_RegisterInt("net_mcast6iface", 0, 0, 65535, CVAR_LATCH | CVAR_ARCHIVE, "IPv6 Network multicast interface");
#else
	net_mcast6iface =
		Cvar_RegisterString("net_mcast6iface", "", CVAR_LATCH | CVAR_ARCHIVE, "IPv6 Network multicast interface");
#endif
	/*
		modified += net_mcast6iface->modified;
		net_mcast6iface->modified = qfalse;

		net_socksEnabled = Cvar_RegisterInt( "net_socksEnabled", 0, 0, 1, CVAR_LATCH | CVAR_ARCHIVE, "Net enable socks
	   proxy support"); modified += net_socksEnabled->modified; net_socksEnabled->modified = qfalse;

		net_socksServer = Cvar_RegisterString( "net_socksServer", "", CVAR_LATCH | CVAR_ARCHIVE, "Net socks proxyserver
	   address"); modified += net_socksServer->modified; net_socksServer->modified = qfalse;

		net_socksPort = Cvar_RegisterInt( "net_socksPort", 1080, 0, 65535, CVAR_LATCH | CVAR_ARCHIVE, "Net socks
	   proxyserver port"); modified += net_socksPort->modified; net_socksPort->modified = qfalse;

		net_socksUsername = Cvar_RegisterString( "net_socksUsername", "", CVAR_LATCH | CVAR_ARCHIVE , "Net socks
	   proxyserver username"); modified += net_socksUsername->modified; net_socksUsername->modified = qfalse;

		net_socksPassword = Cvar_RegisterString( "net_socksPassword", "", CVAR_LATCH | CVAR_ARCHIVE , "Net socks
	   proxyserver password"); modified += net_socksPassword->modified; net_socksPassword->modified = qfalse;
	*/
...
int NET_TcpSendData(int sock, const void *data, int length, char *errormsg, int maxerrorlen)
{
	...
	state =
		send(sock, data, length, NET_NOSIGNAL); // FIX: flag NOSIGNAL prevents SIGPIPE in case of connection problems
	...
	if (ret == SOCKET_ERROR)
	{
		...
		if (ret == ECONNRESET)
		{
			...
			Com_PrintWarningNoRedirect(CON_CHANNEL_NETWORK, "NET_GetTcpPacket recv() syscall failed: %s\n",
				NET_ErrorStringMT(errstr,
					sizeof(errstr))); // BUGFIX: this causes SIGSEGV in case of an error during console stream
...
void NET_TcpServerPacketEventLoop()
{
	...
	for (i = 0, conn = tcpServer.connections; i < MAX_TCPCONNECTIONS; i++, conn++)
	{
		...
		if (FD_ISSET(conn->remote.sock, &fdr) || FD_ISSET(conn->remote.sock, &fdw))
		{
			...
			{
				Com_PrintWarningNoRedirect(CON_CHANNEL_NETWORK,
					"NET_TcpServerPacketEventLoop: Oversize packet from %s. Must not happen!\n",
					NET_AdrToStringMT(&conn->remote, adrstr, sizeof(adrstr)));
...
void NET_TcpServerOpenConnection(netadr_t *from)
{
	...
	for (i = 0, conn = tcpServer.connections; i < MAX_TCPCONNECTIONS; i++, conn++)
	{
		if ((NET_CompareBaseAdr(from, &conn->remote) && conn->state < TCP_AUTHSUCCESSFULL) || conn->remote.sock < 1)
		{ // Net request from same address - Close the old not confirmed connection
			break;
		}
		if (conn->state < TCP_AUTHSUCCESSFULL)
		{
	...
	if (i == MAX_TCPCONNECTIONS)
	{
		if (tcpServer.activeConnectionCount > MAX_TCPCONNECTIONS / 3
			&& oldestTimeAccepted + MAX_TCPCONNECTEDTIMEOUT < NET_TimeGetTime() + 1)
		{
		...
		{
			closesocket(
				from->sock); // We have already too many open connections. Not possible to open more. Possible attack

			if (tcpServer.lastAttackWarnTime + MIN_TCPAUTHWAITTIME < NET_TimeGetTime() + 1)
			{
				tcpServer.lastAttackWarnTime = NET_TimeGetTime() + 1;
				Com_PrintWarning(CON_CHANNEL_NETWORK,
					"Possible Denial of Service Attack, Dropping connectrequest from: %s\n",
...
__optimize3 __regparm3 qboolean NET_TcpServerConnectRequest(netadr_t *net_from, fd_set *fdr)
{
	...
	if (tcp_socket != INVALID_SOCKET && FD_ISSET(tcp_socket, fdr))
	{
		...
		{
			Com_Printf(CON_CHANNEL_NETWORK,
				"WARNING: NET_TcpServerConnectRequest: socket is out of range 0 to 1023. Are there too many open "
...
__optimize3 __regparm1 qboolean NET_TcpServerConnectEvent(fd_set *fdr)
{
	netadr_t from;
	int i;
	// Give the system a possibility to abort processing network packets so it won't block execution of frames if the
...
Or NET_CONNRESET close the socket. Remote host aborted connection connection. [Writes readable message to errormsg if
not NULL] Or NET_ERROR close the socket. An error happened. [Writes readable message to errormsg if not NULL] Or 0 close
the socket. Action completed in most cases.
*/
	...
	int timeoutsec)
{
	...
	if (remoteAdr)
	{
		...
		{
			if (!silent)
				Com_PrintWarning(CON_CHANNEL_NETWORK, "Couldn't resolve: %s\n", remoteAdr);
			return INVALID_SOCKET;
		}
	}
	else
	...
	if (sourceadr)
	{
		...
		if (sourcefam != destfam)
		{
			if (!silent)
				Com_PrintWarning(CON_CHANNEL_NETWORK,
					"NET_TCPConnect: Protocol family mismatch for source and destination\n");
			return INVALID_SOCKET;
		}
	}
	...
	{
		if (!silent)
			Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_TCPConnect: socket: %s\n",
				NET_ErrorStringMT(errstr, sizeof(errstr)));
		return INVALID_SOCKET;
	}
#ifndef _WIN32
	...
	{
		Com_Printf(CON_CHANNEL_NETWORK,
			"WARNING: NET_TCPConnect: socket is out of range 0 to 1023. Are there too many open connections or "
			"files?\n");
		closesocket(newsocket);
		return INVALID_SOCKET;
	}
	...
	{
		if (!silent)
			Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_TCPIPSocket: ioctl FIONBIO: %s\n",
				NET_ErrorStringMT(errstr, sizeof(errstr)));
		closesocket(newsocket);
		return INVALID_SOCKET;
	}
	...
	if (sourceadr)
	{
		...
		{
			if (!silent)
				Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_TcpClientConnect: bind: %s\n",
					NET_ErrorStringMT(errstr, sizeof(errstr)));
			closesocket(newsocket);
			return INVALID_SOCKET;
		}
	...
	)
	{
		...
		case 0:
			if (!silent)
				Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_TcpConnect: Connecting to: %s timed out\n",
					NET_AdrToStringMT(&remoteadr, adrstr, sizeof(adrstr)));
			closesocket(newsocket);
			return INVALID_SOCKET;
		...
		case -1:
			if (!silent)
				Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_TcpConnect: select() syscall failed: %s\n",
					NET_ErrorStringMT(errstr, sizeof(errstr)));
			closesocket(newsocket);
			return INVALID_SOCKET;
	...
	if (!silent)
		Com_PrintWarning(CON_CHANNEL_NETWORK, "NET_TCPOpenConnection: connect error: %s\n",
			NET_ErrorStringMT(errstr, sizeof(errstr)));
	closesocket(newsocket);
	return INVALID_SOCKET;
}
...
__optimize3 __regparm1 qboolean NET_Sleep(unsigned int usec)
{
	...
	else if (retval > 0)
	{
		...
		if (((signed int)tcp_socket != INVALID_SOCKET && FD_ISSET(tcp_socket, &fdr))
			|| ((signed int)tcp6_socket != INVALID_SOCKET && FD_ISSET(tcp6_socket, &fdr)))
		{

...
void IW3_SurfacePlane(q3_dsurface_t *surface, vec3_t normal, float *dist)
{
	...
	*/
	...
	{
		printf("surface %d bogus normal vector %f %f %f\n", surface - iw3_drawSurfaces, normal[0], normal[1],
			normal[2]);
...
{
	int connectingpts = 0;
	if (VectorCompare(tri1->tri[0], tri2->tri[0]) || VectorCompare(tri1->tri[0], tri2->tri[1])
		|| VectorCompare(tri1->tri[0], tri2->tri[2]))
	{
		++connectingpts;
	}
	if (VectorCompare(tri1->tri[1], tri2->tri[0]) || VectorCompare(tri1->tri[1], tri2->tri[1])
		|| VectorCompare(tri1->tri[1], tri2->tri[2]))
	{
		++connectingpts;
	}
	if (connectingpts == 2)
	...
	if (VectorCompare(tri1->tri[2], tri2->tri[0]) || VectorCompare(tri1->tri[2], tri2->tri[1])
		|| VectorCompare(tri1->tri[2], tri2->tri[2]))
	{
		return true;
	}
	return false;
...
void IW3_FindLoneTriangles()
{
	...
	for (i = 0; i < cm.partitionCount; ++i)
	{
		...
		for (triIndex = 0; triIndex < part->numtris; ++triIndex)
		{
			...
			if (IsDegeneratedTriangle(&part->tris[triIndex], part))
			{
				memcpy(&iw3_partitions[cm.partitionCount].tris[iw3_partitions[cm.partitionCount].numtris],
					&part->tris[triIndex], sizeof(iw3_triangle_t));
				part->tris[triIndex].active = false;
...
void IW3_DumpCollisionVerts()
{
	...
	for (i = 0; i < cm.partitionCount; ++i)
	{
		...
		for (triIndex = partition->firstTri, j = 0; triIndex < partition->firstTri + partition->triCount;
			 ++triIndex, ++j)
		{
			float *vect;
			uint16_t *indices = &cm.triIndices[3 * triIndex];
...
int IW3_DumpLightMaps()
{
	...
	for (i = 0; i < s_world.lightmapCount; ++i, ++in)
	{
		...
		printf("%d %d %d Pri Lightmap %s size %d\n", pritex->dimensions[0], pritex->dimensions[1],
			pritex->dimensions[2], primary->name, pritex->resourceSize);
		printf("%d %d %d Sec Lightmap %s size %d\n", sectex->dimensions[0], sectex->dimensions[1],
			sectex->dimensions[2], secondary->name, sectex->resourceSize);
...
void IW3_LoadFastfile(struct quakefile_s *qf)
{
	...
	iw3_numShaders = IW3_CopyLump(cm.materials, cm.numMaterials, (void *)&iw3_dshaders,
		sizeof(q3_dshader_t)); // Called materials in IWx
	iw3_nummodels = IW3_DumpSubmodels(&iw3_dmodels, sizeof(q3_dmodel_t));
	iw3_numplanes = IW3_DumpPlanes(&iw3_dplanes,
		sizeof(q3_dplane_t)); // Important step to do early as leaf and brushsides depend on this
	iw3_numleafs = IW3_DumpLeafs(&iw3_dleafs, sizeof(q3_dleaf_t));
	iw3_numnodes = IW3_DumpNodes(&iw3_dnodes, sizeof(q3_dnode_t));
	iw3_numleafsurfaces =
		IW3_CopyLump(cm.leafsurfaces, cm.numLeafSurfaces, (void *)&iw3_dleafsurfaces, sizeof(iw3_dleafsurfaces[0]));
	...
	iw3_numDrawIndexes =
		IW3_CopyLump(s_world.indices, s_world.indexCount, (void *)&iw3_drawIndexes, sizeof(iw3_drawIndexes[0]));
...
void IW3_WriteBSPFile(char *filename)
{
	...
	IW3_AddLump(bspfile, header, Q3_LUMP_LEAFSURFACES, iw3_dleafsurfaces,
		iw3_numleafsurfaces * sizeof(iw3_dleafsurfaces[0]));
	IW3_AddLump(bspfile, header, Q3_LUMP_LEAFBRUSHES, iw3_dleafbrushes,
		iw3_numleafbrushes * sizeof(iw3_dleafbrushes[0]));

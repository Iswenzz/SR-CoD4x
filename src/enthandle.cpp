...
static void RemoveEntHandleInfo(EntHandleList *entHandleList, unsigned int oldInfoIndex)
{
	...
	if (oldInfoIndex)
	{
		...
		{
			entHandleList->infoIndex =
				oldInfoIndex != g_entHandleInfoArray[oldInfoIndex].next ? g_entHandleInfoArray[oldInfoIndex].next : 0;
		}
...
{
	assertx((unsigned int)number - 1 < ENTITYNUM_NONE, "number - 1 doesn't index ENTITYNUM_NONE - %i not in [0, %i)",
		number - 1, ENTITYNUM_NONE);

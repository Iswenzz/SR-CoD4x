...
{
	string m_strString;
	string m_strDebug; // english and/or "#same", used for debugging only. Also prefixed by "SE:" to show which strings
					   // go through StringEd (ie aren't hardwired)
	int m_iFlags;
...
char *CStringEdPackage::Filename_WithoutExt(LPCSTR psFilename)
{
	...
	// special check, make sure the first suffix we found from the end wasn't just a directory suffix (eg on a path'd
	// filename with no extension anyway)
	//
	if (p && (p2 == 0 || (p2 && p > p2)) && (p3 == 0 || (p3 && p > p3)))
		*p = 0;
...
// returns SE_TRUE if supplied keyword found at line start (and advances supplied ptr past any whitespace to next arg
// (or line end if none),
//
...
LPCSTR CStringEdPackage::InsideQuotes(LPCSTR psLine)
{
	...
	if (psLine[0])
	{
		...
		//
		while (str.c_str()[strlen(str.c_str()) - 1] == ' ' || str.c_str()[strlen(str.c_str()) - 1] == '\t')
		{
...
void CStringEdPackage::AddFlagReference(LPCSTR psLocalReference, LPCSTR psFlagName)
{
	...
	//
	mapStringEntries_t::iterator itEntry =
		m_StringEntries.find(va("%s_%s", m_strCurrentFileRef_ParseOnly.c_str(), psLocalReference));
...
LPCSTR CStringEdPackage::ParseLine(LPCSTR psLine, bool forceEnglish)
{
	...
	if (psLine)
	{
		...
		else if (CheckLineForKeyword(sSE_KEYWORD_CONFIG, psLine) || CheckLineForKeyword(sSE_KEYWORD_FILENOTES, psLine)
			|| CheckLineForKeyword(sSE_KEYWORD_NOTES, psLine) || CheckLineForKeyword(sSE_KEYWORD_FLAGS, psLine))
		{
		...
		{
			// LANG_ENGLISH 	"GUARD:  Good to see you, sir.  Taylor is waiting for you in the clean tent.  We need to
			// get you suited up.  "
			//
			...
			if (psReference[0])
			{
				...
				strncpy(sThisLanguage, psLine,
					iCharsToCopy); // already declared as {0} so no need to zero-cap dest buffer
				...
				//
				SE_BOOL bSentenceIsEnglish = (!Q_stricmp(sThisLanguage, "english"))
					? SE_TRUE
					: SE_FALSE; // see whether this is the english master or not
...
LPCSTR Leetify(LPCSTR psString)
{
	...
	{
		static const char cReplace[] = {
			'o', '0', 'l', '1', 'e', '3', 'a', '4', 's', '5', 't', '7', 'i', '!', 'h', '#', 'O', '0', 'L', '1', 'E',
			'3', 'A', '4', 'S', '5', 'T', '7', 'I', '!', 'H', '#' // laziness because of strchr()
		};
...
extern "C"
{
	LPCSTR SE_Load(LPCSTR psFileName, bool forceEnglish)
	{
		...
		if (psLoadedFile)
		{
			...
			{
				psErrorMessage =
					va("Truncated file, failed to find \"%s\" at file end in file %s", "ENDMARKER", psFileName);
			}
			return psErrorMessage;
		}
	...
	LPCSTR SE_GetString_LoadObj(LPCSTR psPackageAndStringReference)
	{
		...
		// should never get here, but fall back anyway... (except we DO use this to see if there's a debug-friendly key
		// bind, which may not exist)
		//
		//	__ASSERT(0);

...
byte *__cdecl _PMem_AllocNamed(unsigned int size, unsigned int alignment, unsigned int type, unsigned int allocType,
	const char *name, EMemTrack memTrack)
{
	// int alignedSize;
...
byte *__cdecl _PMem_Alloc(unsigned int size, unsigned int alignment, unsigned int type, unsigned int allocType,
	EMemTrack memTrack, const char *file, int lineNum)
{
	char allocIdBuf[256];
	...
	void PMem_DumpMemStats()
	{
		...
		if (g_mem.prim[1].allocListCount)
		{
			for (i = 0; i < (signed int)g_mem.prim[1].allocListCount; ++i)
			{
				...
				Com_Printf(CON_CHANNEL_SYSTEM, "%-18.18s %5.1f\n", g_mem.prim[1].allocList[i].name,
					(float)((signed int)(g_mem.prim[1].allocList[i].pos - h) * 0.00000095367432));
			}
		}
		...
		{
			Com_Printf(CON_CHANNEL_SYSTEM, "%-18.18s %5.1f\n", g_mem.prim[0].allocList[i].name,
				(double)(0.00000095367432 * (long double)(signed int)(h - g_mem.prim[0].allocList[i].pos)));

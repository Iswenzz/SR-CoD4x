...
static void SV_GetPlayerByHandleInternal(const char* s, clanduid_t* cl)
{
	...
	else
	{
		...
		if (!cl->cl)
		{
			...
			if (!lastfound)
			{ // No exact playermatch found - Now search for partial name matches
				for (i = 0, cl->cl = svs.clients; i < sv_maxclients->integer; i++, cl->cl++)
				{
					if (!cl->cl->state)
					{
	...
	if (!cl->cl && cl->steamid != 0)
	{ // See whether this player is currently onto server
		for (i = 0, cl->cl = svs.clients; i < sv_maxclients->integer; i++, cl->cl++)
		{
...
static void SV_MiniStatus_f(void)
{
	...
	Com_Printf(CON_CHANNEL_DONT_FILTER,
		"num score ping playerid            steamid           name                             address                 "
		"                            FPS XVer\n");
...
static void SV_Status_f(void)
{
	...
	else
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"num score ping playerid            steamid           name                             lastmsg address     "
			"                                         qport rate\n");
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"--- ----- ---- ------------------- ----------------- -------------------------------- ------- "
			"---------------------------------------------------- ----- -----\n");
...
static void Cmd_Undercover_f()
{
	...
	if (invokerclnum < 64 && invokerclnum >= 0)
	{
		...
		{
			Com_Printf(CON_CHANNEL_DONT_FILTER,
				"Error: You can not use the command \"undercover\" when you are alive\n");
			return;
		}
...
char* SV_IsGUID(char* GUID)
{
	...
	while (j < k)
	{
		if (GUID[j] < 0x30 || GUID[j] > 0x66 || (GUID[j] < 0x41 && GUID[j] > 0x39)
...
static void Cmd_BanPlayer_f()
{
	...
	if (!Q_stricmp(Cmd_Argv(0), "banUser") || !Q_stricmp(Cmd_Argv(0), "banClient"))
	{
		if (Cmd_Argc() < 2)
		{
			...
			Com_Printf(CON_CHANNEL_DONT_FILTER,
				"Where user is one of the following: online-playername | online-playerslot | playerid\n");
			Com_Printf(CON_CHANNEL_DONT_FILTER,
				"online-playername can be a fraction of the playername. Playerid is in format [U:V:W]\n");
			return;
		}
	}
	else
...
gothandle:
	...
	{
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Error: You have exceeded the maximum allowed length of 126 characters for the reason\n");
		return;
	}
	...
	if (cl.cl)
	{
		...
		SV_PrintAdministrativeLog("banned player: %s id: %s IP: %s with the following reason: %s", cl.cl->name, psti,
...
static void Cmd_TempBanPlayer_f()
{
	...
	if (Cmd_Argc() < 4)
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Where reason for this ban is contains a description without the letters: \" ; %% / \\ \n");
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"online-playername can be a fraction of the playername. Playerid is in format [U:V:W]");
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"XX is an integer representing the time of ban in minutes, hours or days\n");
		return;
	}
...
gothandle:
	...
	{
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Error: You have exceeded the maximum allowed length of 126 for the reason\n");
		return;
	}
	...
	if (cl.cl)
	{
		...
		SV_PrintAdministrativeLog("temporarily banned player: %s id: %s IP: %s until %s with the following reason: %s",
			cl.cl->name, psti, NET_AdrToString(&cl.cl->netchan.remoteAddress), endtime, banreason);
		Com_sprintf(dropmsg, sizeof(dropmsg),
	...
	else
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER, "Banrecord added for id: %s\n", psti);
		SV_PrintAdministrativeLog("temporarily banned player id: %s until %s with the following reason: %s", psti,
			endtime, banreason);
	}
	// InsertPluginEvent
}
...
static void Cmd_KickPlayer_f()
{
	...
	if (Cmd_Argc() < 2)
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Where user is one of the following: online-playername | online-playerslot\n");
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Where reason for this ban is contains a description without the characters: \" ; %% / \\ (max. 126 "
			"chars)\n");
	...
	if (!Q_strncmp(Cmd_Argv(1), "all", 3) && strlen(Cmd_Argv(1)) == 3)
	{
		...
		for (i = 0; i < sv_maxclients->integer; ++i)
		{
			...
			if (c->state != CS_FREE && c->state != CS_ZOMBIE && c->power < Cmd_GetInvokerPower()
				&& Cmd_GetInvokerPower() > 1)
			{
				SV_SApiSteamIDToString(c->playerid, psti, sizeof(psti));
				Com_Printf(CON_CHANNEL_DONT_FILTER, "Player kicked: %s ^7id: %s\nReason: %s\n", c->name, psti,
					kickreason);
				SV_PrintAdministrativeLog("kicked player: %s^7 id: %s with the following reason: %s", c->name, psti,
					kickreason);
	...
	else // Kick only one player.
	{
		...
		SV_PrintAdministrativeLog("kicked player: %s^7 id: %s with the following reason: %s", cl.cl->name, psti,
			kickreason);
	...
	while (*cmdstring)
	{
		if (*cmdstring == '$')
		{
			...
			else if (!Q_strncmp(cmdstring, "$arg", 4))
			{
				...
				{
					cmdstring += 4;
					if (*cmdstring == ':' && *(cmdstring + 1) != ' ')
					{				 // Default argument in place!
						++cmdstring; // Just advance the pointer and read in the argument as any other part of the
									 // string
					}
					else
					{
...
static void Cmd_AddTranslatedCommand_f()
{
	...
	{
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Usage: addCommand <commandname> <\"string to execute\"> String can contain: $uid $clnum $pow $arg "
			"$arg:default\n");
		return;
	}
...
static void SV_MixMapsInRotation()
{
	...
	struct
	{
		char* gametype;
		char* mapname;
	} rotInfo[CVAR_STRING_SIZE / 8] = { { NULL,
		NULL } }; // Minimum possible correct rotation command: "map mp_z". Len = 8.

	int rotationLen =
		strlen(sv_mapRotation->string) > CVAR_STRING_SIZE ? CVAR_STRING_SIZE : strlen(sv_mapRotation->string);
	...
	for (i = mapCount; i > 0; --i) // Random here. i isn't in use. mapIndex iterations.
	{
		...
		if (Q_stricmp(rotInfo[ri].gametype,
				lastGametype)) // Gametype of previous placed map differs from new one? place 'gametype' keyword.
		{
		...
		if (strlen(mixedList)
			> CVAR_STRING_SIZE - 100) // Exit from safezone. EXTREMELY rare rotation crash: when len(mapname) >= ~90
			break;
	}
...
qboolean SV_RunDownload(const char* url, const char* filename)
{
	...
	{
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Couldn't connect to download-server for downloading. Failed to download %s\n", filename);
		return qfalse;
	}
	...
	{
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Downloading of file: \"%s\" has failed with the following message: %d %s\n", filename, curfileobj->code,
			curfileobj->status);
		FileDownloadFreeRequest(curfileobj);
		return qfalse;
	}
	...
	{
		len = FS_SV_HomeWriteFile(filename, curfileobj->recvmsg.data + curfileobj->headerLength,
...
void SV_MapCompletedExec(const char* mapname)
{
	...
	Com_sprintf(cmdline, sizeof(cmdline), "\"%s/apps/%s\" \"%s/usermaps/%s\"", fs_homepath->string,
		sv_mapDownloadCompletedCmd->string, fs_homepath->string, mapname);
...
void SV_AddOperatorCommands()
{
	...
	Cmd_AddPCommand("sv_kick", Cmd_KickPlayer_f, 35);

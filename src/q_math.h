...
#define vec2_copy(to, from) \
	(to)[0] = (from)[0];    \
	(to)[1] = (from)[1]
#define vec2_add(to, from) \
	(to)[0] += (from)[0];  \
	(to)[1] += (from)[1]
#define vec2_substract(to, from) \
	(to)[0] -= (from)[0];        \
	(to)[1] -= (from)[1]
#define vec2_multiply(v, k) \
	(v)[0] *= (k);          \
	(v)[1] *= (k)
...
#define vec3_copy(to, from)  \
	vec2_copy((to), (from)); \
	(to)[2] = (from)[2]
#define vec3_add(to, from)  \
	vec2_add((to), (from)); \
	(to)[2] += (from)[2]
#define vec3_substract(to, from)  \
	vec2_substract((to), (from)); \
	(to)[2] -= (from)[2]
#define vec3_multiply(v, k)  \
	vec2_multiply((v), (k)); \
	(v)[2] *= (k)

...
void __cdecl Scr_ValidateLocalizedStringRef(int parmIndex, const char *token, int tokenLen)
{
	...
	if (tokenLen > 1)
	{
		for (charIter = 0; charIter < tokenLen; ++charIter)
		{
			...
			{
				Scr_ParamError(parmIndex,
					va("Illegal localized string reference: %s must contain only alpha-numeric characters and "
					   "underscores",
						token));
			}
		}
...
extern "C"
{
	void __cdecl GScr_PlaceSpawnPoint(scr_entref_t entref)
	{
		...
		if (trace.allsolid)
		{
			Com_PrintWarning(CON_CHANNEL_PARSERSCRIPT, "WARNING: Spawn point entity %i is in solid at (%i, %i, %i)\n",
				pEnt->s.number, (signed int)pEnt->r.currentOrigin[0], (signed int)pEnt->r.currentOrigin[1],
	...
	void __cdecl Scr_ConstructMessageString(int firstParmIndex, int lastParmIndex, const char *errorContext,
				...
				for (charIndex = 0; charIndex < tokenLen; ++charIndex)
				{
					...
					if (isalpha(token[charIndex]))
					{
						if (loc_warnings->boolean)
						{
							...
							{
								Scr_LocalizationError(parmIndex,
							...
							else
							{
								Com_PrintWarning(CON_CHANNEL_PLAYERWEAP,
									"WARNING: Non-localized %s string is not allowed to have letters in it. Must be "
	...
	void __cdecl Scr_PlayerKilled(gentity_s *self, gentity_s *inflictor, gentity_s *attacker, int damage,
		int meansOfDeath, int iWeapon, const float *vDir, hitLocation_t hitLoc, int psTimeOffset, int deathAnimDuration)
	{
	...
	void __cdecl Scr_PlayerLastStand(gentity_s *self, gentity_s *inflictor, gentity_s *attacker, int damage,
		int meansOfDeath, int iWeapon, const float *vDir, hitLocation_t hitLoc, int psTimeOffset)
	{
		uint16_t cstr;
		const char *weapname;
		uint16_t callback;

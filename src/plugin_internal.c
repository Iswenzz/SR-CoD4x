...
void PHandler_TcpCloseConnection(int pID, int connection)
{
	...
	if (ptcs->sock < 1)
	{
		Com_PrintWarning(CON_CHANNEL_PLUGINS,
			"Plugin_TcpCloseConnection: Called on a non open socket for plugin ID: #%d\n", pID);
		return;
	}
	NET_TcpCloseSocket(ptcs->sock);
...
void PHandler_CmdExecute_f()
{
	...
	if (!pluginFunctions.enabled)
	{
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Error! Tried executing a plugin command with plugins being disabled! Command name: '%s'.\n", Cmd_Argv(1));
		return;
	}
	char name[128];
	int i, j;
	...
	for (i = 0; i < MAX_PLUGINS; i++)
	{
		if (pluginFunctions.plugins[i].loaded && pluginFunctions.plugins[i].enabled)
		{
			for (j = 0; j < pluginFunctions.plugins[i].cmds; j++)
			{
				...
				{
					//                    Com_Printf(CON_CHANNEL_PLUGINS,"Executing plugin command '%s' for plugin '%s',
...
void PHandler_Free(int pID, void *ptr)
{
	...
	{
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Plugins: Warning! Plugin #%d tried freeing a NULL pointer! Called Plugin_Free() twice?\n", pID);
		return;
	}
	// Plugin identified, find the first free spot in it's allocated pointers table
	for (i = 0; i < PLUGIN_MAX_MALLOCS; i++)
...
P_P_F void PHandler_Error(int pID, int code, char *string)
{
	...
	{
		Com_PrintWarning(CON_CHANNEL_PLUGINS,
			"An error of ID %d and string \"%s\" occured in a disabled plugin with ID %d!\n", code, string, pID);
		return;
	}
	switch (code)
	{
	...
	case P_ERROR_DISABLE:
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Plugin #%d ('%s') returned an error and will be disabled! Error string: \"%s\".\n", pID,
			pluginFunctions.plugins[pID].name, string);
		pluginFunctions.plugins[pID].enabled = qfalse;
		break;
	case P_ERROR_TERMINATE:
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Plugin #%d ('%s') reported a critical error, the server will be terminated. Error string: \"%s\".\n", pID,
			pluginFunctions.plugins[pID].name, string);
		Com_Error(ERR_FATAL, "%s", string);
		break;
	default:
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Plugin #%d ('%s') reported an unknown error! Error string: \"%s\", error code: %d.\n", pID,
			pluginFunctions.plugins[pID].name, string, code);
		break;
	}
}
/*
...
void PHandler_PluginList_f()
{
	...
	else
	{
		...
		for (i = 0, j = 0; i < pluginFunctions.loadedPlugins; ++i, ++j)
		{
			...
			Com_Printf(CON_CHANNEL_PLUGINS, "| %-3d| %-21s| %-9s| %-19d| %-21d|\n", j, pluginFunctions.plugins[j].name,
				pluginFunctions.plugins[j].enabled == 0 ? "no" : "yes", pluginFunctions.plugins[j].mallocs,
...
int PHandler_CallerID() // Can now be inlined, why not ^^
{
	...
	if(j<3){
	Com_Error(ERR_FATAL,"PHandler_CallerID: backtrace failed to return function pointers! Possible exploit detected!
	Terminating the server...\n");
	}
	for(i=0;i<MAX_PLUGINS;++i){
		if(pluginFunctions.plugins[i].lib_start < funcptrs[2] && pluginFunctions.plugins[i].lib_start +
	pluginFunctions.plugins[i].lib_size > funcptrs[2]) return i;
	}
	return -1;*/
}
...
void PHandler_ScrAddFunction(char *name, xfunction_t function, qboolean replace, int pID)
{
	...
	{
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Error: tried adding a script command for a plugin with non existent pID. pID supplied: %d.\n", pID);
		return;
	}
	else if (pID < 0)
	{
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Plugin Handler error: Plugin_ScrAddFunction or Plugin_ScrReplaceFunction called from not within a plugin "
			"or from a disabled plugin!\n");
		return;
	}
	...
	if (Scr_AddFunction(name, (xfunction_t)&pluginScriptCallStubs.s[pID * MAX_SCRIPTFUNCTIONS + i].dyncallstub, qfalse)
		== qfalse)
	{
	...
	Q_strncpyz(pluginScriptCallStubs.s[pID * MAX_SCRIPTFUNCTIONS + i].name, name,
		sizeof(pluginScriptCallStubs.s[pID * MAX_SCRIPTFUNCTIONS + i].name));
...
void PHandler_ScrAddMethod(char *name, xfunction_t function, qboolean replace, int pID)
{
	...
	{
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Plugin Handler error: Plugin_ScrAddMethod or Plugin_ScrReplaceMethod called from not within a plugin or "
	...
	if (Scr_AddMethod(name, (xfunction_t)&pluginScriptCallStubs.s[pID * MAX_SCRIPTFUNCTIONS + i].dyncallstub, qfalse)
		== qfalse)
	{

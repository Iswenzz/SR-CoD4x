...
const char *propertyNames[] = { "loopsync", "nonloopsync", "complete", "additive", "client", "separate", "forceload" };
...
extern "C"
{
	...
	bool __regparm3 AnimTreeParseInternal(unsigned int parentNode, unsigned int names, bool bIncludeParent, bool bLoop,
		bool bComplete)
	{
		...
		while (1)
		{
			...
			if (Scr_IsIdentifier(tok->token))
			{
				...
				if (tok->token[0] != '{' || tok->token[1])
				{
					token_pos = Com_GetLastTokenPos();
					Com_EndParseSession();
					CompileError(token_pos - gScrAnimGlob.start, "%s",

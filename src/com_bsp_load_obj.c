...
{
	if (comBspGlob.header->ident == (uint32_t)'PSBI' && comBspGlob.header->version >= 6
...
void __cdecl Com_LoadBsp(const char *filename)
{
	...
	assert(len < (sizeof(comBspGlob.name)
			   / (sizeof(comBspGlob.name[0]) * (sizeof(comBspGlob.name) != 4 || sizeof(comBspGlob.name[0]) <= 4))));
...
byte *__cdecl Com_ValidateBspLumpData(enum LumpType type, unsigned int offset, unsigned int length,
	unsigned int elemSize, unsigned int *count)
{
	assert(count);
...
byte *__cdecl Com_GetBspLump(enum LumpType type, unsigned int elemSize, unsigned int *count)
{
	...
	if (comBspGlob.header->version > 18)
	{
		...
		for (chunkIter = 0; chunkIter < comBspGlob.header->chunkCount; ++chunkIter)
		{
			...
			{
				return Com_ValidateBspLumpData(type, offset, comBspGlob.header->chunks[chunkIter].length, elemSize,
					count);
			}
	...
	{
		return Com_ValidateBspLumpData(type, comBspGlob.header->chunks[type].type,
			*(&comBspGlob.header->chunkCount + 2 * type), elemSize, count);
	}
	*count = 0;
	return NULL;
}

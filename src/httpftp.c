...
static int FT_SendData(ftRequest_t* request)
{
	...
	if (request->tls == NULL)
	{
		bytes = NET_TcpSendData(request->socket, request->sendmsg.data, request->sendmsg.cursize, errormsg,
			sizeof(errormsg));
...
int HTTP_TcpReceiveData(ftRequest_t* request)
{
	...
	if (request->tls == NULL)
	{
		ret = NET_TcpClientGetData(request->socket, request->recvmsg.data + request->recvmsg.cursize, len, errormsg,
			sizeof(errormsg));
	}
	else
	{
	...
	if ((request->tls == NULL && ret == NET_WANT_READ)
		|| (request->tls && (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE)))
	{
		return 1;
	}
	NET_TcpCloseSocket(request->socket);
			...
			&& line[0] != '\r')
		{
			...
			else if (!Q_stricmpn("Location:", line, 9))
			{
				if (strlen(line + 9) > 8)
				{
					...
					qboolean tls = HTTPSplitURL(line, request->address, sizeof(request->address), request->url,
						sizeof(request->url));
...
static int HTTPS_ReadCertificatesFromZipFile(void* cacert,
	int (*store_callback)(void* ca_ctx, const unsigned char* pemderbuf, int lenofpemder))
{
	unzFile uzf;
	char zipfile[1024];
...
static int HTTPS_SetupCAs()
{
	...
	if (Sys_ReadCertificate((void*)&cacert, LoadSingleCertificateCallback) < 1) // Reading system root certificates
	{
		Com_Printf(CON_CHANNEL_FILEDL,
			"InitCA: to parse system root certificates. On Linux you can set environment variable SSL_CERT_FILE to the "
			"file path pointing on your root certificates\n");
		Com_Printf(CON_CHANNEL_FILEDL, "InitCA Attempting to read certificates from file main/xbase_00.iwd\n");
		if (HTTPS_ReadCertificatesFromZipFile((void*)&cacert, LoadSingleCertificateCallback)
			!= 1) // Reading root certificates from iwd file
		{
			Com_Printf(CON_CHANNEL_FILEDL,
				"InitCA fatal error: Did not read any certificates. Some functions using HTTPS are not going to "
				"work.\n");
			Sys_LeaveCriticalSection(CRITSECT_HTTPS);
			return 0;
		}
...
static int HTTPS_Prepare(ftRequest_t* request, const char* commonname)
{
	...
	if ((ret = mbedtls_ctr_drbg_seed(&request->tls->ctr_drbg, mbedtls_entropy_func, &request->tls->entropy,
			 (const unsigned char*)HTTP_CLIENTNAME, strlen(HTTP_CLIENTNAME)))
		!= 0)
	{
		mbedtls_strerror(ret, errormsg, sizeof(errormsg));
	...
	if ((ret = mbedtls_ssl_config_defaults(&request->tls->conf, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
...
*/
static void FTP_SplitURL(const char* url, char* address, int lenaddress, char* path, int lenpath, char* loginname,
...
static int FTP_ReceiveData(ftRequest_t* request)
{
	...
	status = NET_TcpClientGetData(request->transfersocket, request->transfermsg.data + request->transfermsg.cursize,
		len, NULL, 0);
	...
	while ((line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf, sizeof(stringlinebuf))) && line[0] != '\0')
	{
		...
		case 227:
			if (request->stage == 36)
			{
				...
				{
					Com_PrintWarning(CON_CHANNEL_FILEDL,
						"FTP_SendReceiveData: Couldn't read the address/port of passive mode response\n");
					return -1;
				}
			}
			else
		...
		case 213:
			if (request->stage == 50)
			{
				...
				{
					Com_PrintWarning(CON_CHANNEL_FILEDL,
						"FTP_SendReceiveData: Failed to allocate %d bytes for download file!\n", bytes);
					return -1;
				}
		...
		case 503:
			if (request->stage == 32)
			{
				request->stage = 35;
				Com_DPrintf(CON_CHANNEL_FILEDL,
					"FTP_SendReceiveData: Command EPSV is not implemented on FTP server. Trying PASV...\n");
				break;
			}
			else if (request->stage == 36)
			{
				Com_Printf(CON_CHANNEL_FILEDL,
					"FTP_SendReceiveData: FTP Server does not support passive mode. Request failed!\n");
				request->stage = -10;
			}
		default:
...
const char* FileDownloadGenerateProgress(ftRequest_t* request)
{
	...
	Com_sprintf(line, sizeof(line), "\rReceiving: %s %.1f/%.1f kBytes @ %.1f kB/s", bar, (float)msg->cursize / 1024.0f,
		(float)msg->maxsize / 1024.0f, rate);

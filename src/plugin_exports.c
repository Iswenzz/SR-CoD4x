...
P_P_F void Plugin_AddCommand(char* name, xcommand_t xcommand, int power)
{
	...
	if (pID >= MAX_PLUGINS)
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Tried adding a command for a plugin with non existent pID. pID supplied: %d.\n", pID);
		return;
	}
	else if (pID < 0)
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_AddCommand called from not within a plugin or from a disabled plugin!\n");
		return;
	}
...
P_P_F void Plugin_RemoveCommand(char* name)
{
	...
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Tried removing a command for a plugin with non existent pID. pID supplied: %d.\n", pID);
		return;
	}
	else if (pID < 0)
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_RemoveCommand called from not within a plugin or from a disabled plugin!\n");
		return;
	}
	if (!pluginFunctions.plugins[pID].loaded)
	{
...
P_P_F qboolean Plugin_TcpConnectMT(int pID, int connection, const char* remote)
{
	...
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_TcpConnect: Second argument can only be a value inside the range: 0...%d plugin ID: #%d\n",
			PLUGIN_MAX_SOCKETS);
		return qfalse;
	}
...
P_P_F int Plugin_TcpGetDataMT(int pID, int connection, void* buf, int size)
{
	...
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_TcpGetData: First argument can only be a value inside the range: 0...%d plugin ID: #%d\n",
	...
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_TcpGetData: Third argument can not be a NULL-Pointer for plugin ID: #%d\n", pID);
		return -1;
	}
...
P_P_F qboolean Plugin_TcpSendDataMT(int pID, int connection, void* data, int len)
{
	...
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_TcpSendData: Second argument can only be a value inside the range: 0...%d plugin ID: #%d\n",
			PLUGIN_MAX_SOCKETS, pID);
		return qfalse;
	}
	if (data == NULL)
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_TcpSendData: Second argument can not be a NULL-Pointer for plugin ID: #%d\n", pID);
		return qfalse;
	}
...
P_P_F void Plugin_TcpCloseConnectionMT(int pID, int connection)
{
	...
	if (pID < 0 || pID >= MAX_PLUGINS)
	{
		Com_Printf(CON_CHANNEL_PLUGINS,
			"Plugin_TcpCloseConnection: Error! Tried get close a connection for unknown plugin!\n");
		return;
	}
	if (pluginFunctions.plugins[pID].enabled == qfalse)
	{
		Com_Printf(CON_CHANNEL_PLUGINS,
			"^1WARNING^7: Plugin_TcpCloseConnection: Tried to close a connection for a disabled plugin!\n");
		return;
	}
	if (connection >= PLUGIN_MAX_SOCKETS || connection < 0)
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_TcpCloseConnection: Second argument can only be a value inside the range: 0...%d plugin ID: #%d\n",
			PLUGIN_MAX_SOCKETS, pID);
		return;
	}
...
P_P_F qboolean Plugin_UdpSendData(netadr_t* to, void* data, int len)
{
	...
	if (to == NULL)
	{
		pID = PHandler_CallerID();
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_UdpSendData: First argument can not be a NULL-Pointer for plugin ID: #%d\n", pID);
		return qfalse;
	}
...
{
	Com_PrintError(CON_CHANNEL_PLUGINS,
		"Plugin_SetPlayerGUID: This command is in CoD4X18+ deprecated. Nothing is set\n");
}

P_P_F void Plugin_SetPlayerUID(unsigned int clientslot, unsigned int uid)
{
	Com_PrintError(CON_CHANNEL_PLUGINS,
		"Plugin_SetPlayerUID: This command is in CoD4X18+ deprecated. Nothing is set\n");
	return;
}
...
{
	Com_PrintError(CON_CHANNEL_PLUGINS,
		"Plugin_SetPlayerNoPB: This command is in CoD4X18+ deprecated. Nothing is set\n");
	return;
}
...
P_P_F void Plugin_BanClient(unsigned int clientnum, int duration, int invokerid, char* banreason)
{
	...
	SV_PrintAdministrativeLog("Banned player: %s id: %s until %s with the following reason: %s", cl->name,
		playeridstring, endtime, banreason);
...
P_P_F ftRequest_t* Plugin_HTTP_MakeHttpRequest(const char* url, const char* method, byte* requestpayload,
...
/* blocking */
P_P_F ftRequest_t* Plugin_HTTP_Request(const char* url, const char* method, byte* requestpayload, int payloadlen,
	...
	for (j = 0; j < MAX_PLUGINS; ++j)
	{
		...
		for (tcb = pluginFunctions.plugins[j].thread_callbacks, i = 0; i < MAX_PLUGINCALLBACKS; i++)
		{
			...
			if (tcb->callbackMain != NULL)
				tcb->callbackMain(tcb->callback_args[0], tcb->callback_args[1], tcb->callback_args[2],
					tcb->callback_args[3], tcb->callback_args[4], tcb->callback_args[5], tcb->callback_args[6],
					tcb->callback_args[7]);
...
P_P_F qboolean Plugin_SetupThreadCallback(void* callbackMain, ...)
{
	...
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_SetupThreadCallback: Can not call this function from the main-thread!\n");
		return qfalse;
	}
...
P_P_F qboolean Plugin_CreateCallbackThread(void* threadMain, ...)
{
	...
	{
		Com_PrintError(CON_CHANNEL_PLUGINS,
			"Plugin_CreateCallbackThread: Can only call this function from the main-thread!\n");
		return qfalse;
	}

...
void __cdecl CMod_LoadNodes()
{
	...
	while (nodeIter < count)
	{
		Com_Printf(CON_CHANNEL_SYSTEM, "Node %d, plane %d, childs %d %d, mins %g %g %g, maxs %g %g %g\n", nodeIter,
			in->planeNum, in->children[0], in->children[1], in->mins[0], in->mins[1], in->mins[2], in->maxs[0],
			in->maxs[1], in->maxs[2]);
...
void __cdecl CMod_LoadCollisionPartitions_VersionBlackOps( )
{
  ...
  in = (DiskCollPartition_VersionBlackOps *)Com_GetBspLump(LUMP_COLLISIONPARTITIONS,
sizeof(DiskCollPartition_VersionBlackOps), &count); cm.partitions = (CollisionPartition
*)CM_Hunk_Alloc(sizeof(CollisionPartition) * count, "CMod_LoadCollisionPartitions"); cm.partitionCount = count; out =
cm.partitions; index = 0; while ( index < count )
  {
...
void __cdecl CMod_LoadCollisionPartitions()
{
	...
	cm.partitions =
		(CollisionPartition *)CM_Hunk_Alloc(sizeof(CollisionPartition) * count, "CMod_LoadCollisionPartitions");
...
void CMod_LoadVisibility()
{
	...
	assertx(len == (unsigned)cm.numClusters * (unsigned)cm.clusterBytes + VIS_HEADER, "%i != %i == %i * %i + %i", len,
		cm.clusterBytes * cm.numClusters + VIS_HEADER, cm.numClusters, cm.clusterBytes, VIS_HEADER);
...
const char *g_purgeableEnts[] = { "misc_model", "misc_prefab", "dyn_brushmodel", "dyn_model", "reflection_probe",
	"info_null", "func_group" };

bool __cdecl MapEnts_CanPurgeEntity(const char *classname, void *userData,
...
double __cdecl CMod_GetPartitionScore(uint16_t *leafBrushes, int numLeafBrushes, int axis, const float *mins,
	const float *maxs, float *dist)
{
	signed int bc;
	float max;
	int rightBrushCount;
...
cLeafBrushNode_s *__cdecl CMod_PartionLeafBrushes_r(uint16_t *leafBrushes, int numLeafBrushes, const float *mins,
	const float *maxs)
{
	int nodeOffset;

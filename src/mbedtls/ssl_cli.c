...
static void ssl_write_encrypt_then_mac_ext(mbedtls_ssl_context *ssl, unsigned char *buf, size_t *olen)
{
	...
	*olen = 0;
	...
	MBEDTLS_SSL_DEBUG_MSG(3,
		("client hello, adding encrypt_then_mac "
		 "extension"));
...
static void ssl_write_extended_ms_ext(mbedtls_ssl_context *ssl, unsigned char *buf, size_t *olen)
{
	...
	*olen = 0;
	...
	MBEDTLS_SSL_DEBUG_MSG(3,
		("client hello, adding extended_master_secret "
		 "extension"));
...
static int ssl_write_client_hello(mbedtls_ssl_context *ssl)
{
	...
	*q++ = (unsigned char)(n << 1);
	...
	if (offer_compress)
	{
		...
		MBEDTLS_SSL_DEBUG_MSG(3,
			("client hello, compress alg.: %d %d", MBEDTLS_SSL_COMPRESS_DEFLATE, MBEDTLS_SSL_COMPRESS_NULL));
...
static int ssl_parse_renegotiation_info(mbedtls_ssl_context *ssl, const unsigned char *buf, size_t len)
{
	...
	if (ssl->renego_status != MBEDTLS_SSL_INITIAL_HANDSHAKE)
	{
		...
		if (len != 1 + ssl->verify_data_len * 2 || buf[0] != ssl->verify_data_len * 2
			...
			|| mbedtls_ssl_safer_memcmp(buf + 1 + ssl->verify_data_len, ssl->peer_verify_data, ssl->verify_data_len)
				!= 0)
		{
		...
		MBEDTLS_SSL_DEBUG_MSG(1,
			("server version out of bounds - "
			 " min: [%d:%d], server: [%d:%d], max: [%d:%d]",
				ssl->conf->min_major_ver, ssl->conf->min_minor_ver, ssl->major_ver, ssl->minor_ver,
...
static int ssl_parse_server_dh_params(mbedtls_ssl_context *ssl, unsigned char **p, unsigned char *end)
{
	...
	{
		MBEDTLS_SSL_DEBUG_MSG(1,
			("DHM prime too short: %d < %d", ssl->handshake->dhm_ctx.len * 8, ssl->conf->dhm_min_bitlen));
...
static int ssl_write_encrypted_pms(mbedtls_ssl_context *ssl, size_t offset, size_t *olen, size_t pms_offset)
{
	...
	if ((ret = mbedtls_pk_encrypt(&ssl->session_negotiate->peer_cert->pk, p, ssl->handshake->pmslen,
			 ssl->out_msg + offset + len_bytes, olen, MBEDTLS_SSL_OUT_CONTENT_LEN - offset - len_bytes,
			 ssl->conf->f_rng, ssl->conf->p_rng))
		!= 0)
	{
...
start_processing:
	...
	if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info))
	{
		...
		if ((ret = mbedtls_pk_verify_restartable(&ssl->session_negotiate->peer_cert->pk, md_alg, hash, hashlen, p,
				 sig_len, rs_ctx))
			!= 0)
		{
		#if defined(MBEDTLS_SSL__ECP_RESTARTABLE)
...
static int ssl_parse_certificate_request(mbedtls_ssl_context *ssl)
{
	...
	if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3)
	{
		...
		{
			MBEDTLS_SSL_DEBUG_MSG(3,
				("Supported Signature Algorithm found: %d"
				 ",%d",
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context *ssl)
{
	...
	if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA)
	{
		...
		ret = mbedtls_dhm_make_public(&ssl->handshake->dhm_ctx, (int)mbedtls_mpi_size(&ssl->handshake->dhm_ctx.P),
			...
			|| ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA)
	{
		...
		ret = mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx, &n, &ssl->out_msg[i], 1000, ssl->conf->f_rng,
			ssl->conf->p_rng);
		if (ret != 0)
		{
		...
		if (mbedtls_ssl_ciphersuite_uses_psk(ciphersuite_info))
	{
			...
			if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_PSK)
		{
			...
			{
				MBEDTLS_SSL_DEBUG_MSG(1,
					("psk identity or DHM size too long"
			...
			if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK)
		{
			 ...
			 */
			ret = mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx, &n, &ssl->out_msg[i],
		...
		if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE)
	{
		...
		ret = mbedtls_ecjpake_write_round_two(&ssl->handshake->ecjpake_ctx, ssl->out_msg + i,
			MBEDTLS_SSL_OUT_CONTENT_LEN - i, &n, ssl->conf->f_rng, ssl->conf->p_rng);
		if (ret != 0)
		{
...
sign:
	...
	if ((ret = mbedtls_pk_sign_restartable(mbedtls_ssl_own_key(ssl), md_alg, hash_start, hashlen,
			 ssl->out_msg + 6 + offset, &n, ssl->conf->f_rng, ssl->conf->p_rng, rs_ctx))
		!= 0)
	{

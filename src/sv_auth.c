...
static void Auth_SetAdmin_f()
{
	...
	if (Cmd_Argc() != 3 || power < 1 || power > 100)
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Where user is one of the following: online-playername | online-playerslot | SteamId\n");
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Don't create non admin accounts (VIP) with a level of 10 or more points\n");
		return;
	}
	...
	if (Cmd_GetInvokerPower() != 100 && power >= Cmd_GetInvokerPower())
	{
		...
		Com_PrintError(CON_CHANNEL_DONT_FILTER,
			"Creating an admin which has more or equal to your own powerpoints is forbidden :)\n");
		Com_Printf(CON_CHANNEL_DONT_FILTER, "You should be able to use Rcon if you are permitted to do this.\n");
		return;
	}
	NV_ProcessBegin();
...
void Auth_UnsetAdmin_f(void)
{
	...
	if (Cmd_Argc() != 2)
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Note: Use the command \"AdminListAdmins\" to get a list of known admins\n");
...
void Auth_ChangeOwnPassword_f()
{
	...
	{
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"This command can not be used from this place\nYou have no account it seems\n");
		return;
	}
...
qboolean Auth_AddAdminToList(const char* username, const char* password, const char* salt, int power, uint64_t steamid,
	qboolean undercover)
{
	authData_admin_t* user;
	authData_admin_t* free = NULL;
	...
	if (steamid == 0 || !username || !*username || power < 0
		|| power > 100 /*!password || strlen(password) < 6 || !salt || strlen(salt) != 64*/)
		return qfalse;
...
static void Auth_SetCommandPower_f()
{
	...
	if (Cmd_Argc() != 3 || atoi(Cmd_Argv(2)) < 1 || atoi(Cmd_Argv(2)) > 100)
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER, "Where power is one of the following: Any number between 1 and 100\n");
		Com_Printf(CON_CHANNEL_DONT_FILTER,
			"Where command is any command you can invoke from console / rcon but no cvars\n");
		return;
	}
...
qboolean SV_RemoteCmdAddAdmin(int uid, char* guid, int power)
{
	...
	if(power < 1 || power > 100){
		Com_Printf(CON_CHANNEL_DONT_FILTER,"Error: Invalid powerlevel(%i). Powerlevel can not be less than 1 or greater
than 100.\n"); return qfalse;
	}

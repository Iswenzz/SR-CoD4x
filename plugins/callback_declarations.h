...
PCL void OnPlayerKilled(gentity_t* self, gentity_t* inflictor, gentity_t* attacker, int damage, int meansOfDeath,
	int iWeapon, hitLocation_t hitLocation);
...
// Following function gets called when the player has basic authentication completed. Here we got a pbguid, the uid
// which is a short steamid, rejectmsg can be written a text to but never exceed 1023 characters, returnNow will keep
// the client idle when set to true, client_t*
PCL void OnPlayerGotAuthInfo(netadr_t* from, uint64_t* playerid, uint64_t* steamid,
	char* rejectmsg /*never exceed a limit of 1023 chars*/, qboolean* returnNow,
	client_t* cl); // an unverified playerid, SteamID "steamIdPending" and "clanIdPending" is not getting verified yet
				   // at all - you can get this info from client_t* structure
...
PCL void OnTerminate(); // Plugins using threads have to listen to this and terminate all threads without keeping
						// critical sections locked. Plugins are susposed not to return from this functions until all
						// child threads are terminated

...
__cdecl CONVAR_T *Plugin_Cvar_RegisterString(const char *var_name, const char *var_value, int flags,
	const char *var_description);
__cdecl CONVAR_T *Plugin_Cvar_RegisterBool(const char *var_name, qboolean var_value, int flags,
	const char *var_description);
__cdecl CONVAR_T *Plugin_Cvar_RegisterInt(const char *var_name, int var_value, int min_value, int max_value, int flags,
	const char *var_description);
//    __cdecl CONVAR_T* Plugin_Cvar_RegisterEnum(const char *var_name, char** valnames, int defaultval, int flags, const
//    char *var_description);
__cdecl CONVAR_T *Plugin_Cvar_RegisterFloat(const char *var_name, float var_value, float min_value, float max_value,
	int flags, const char *var_description);
...
// __cdecl const char* Plugin_Cvar_VariableString( const char *var_name ); deprecated. Use
// Plugin_Cvar_VariableStringBuffer Sets a cvar by name and by a string value which gets interpreted correctly depending
// on the cvar type
__cdecl void Plugin_Cvar_Set(const char *var_name, const char *value);

//      == File handling functions == - Do we really need those?

__cdecl long Plugin_FS_SV_FOpenFileRead(const char *filename,
	fileHandle_t *fp); // Open a file for reading. First from fs_homepath. If not found from fs_baspath. Returns length
					   // of file which should be greater than 0. Writes into *fp the filehandle. Needs to be closed
					   // with Plugin_FS_FCloseFile() once operation is done.
/*
 Example code:
 fileHandle_t fh;
 long len;
 ...
 Plugin_FS_FCloseFile(fh);
*/
__cdecl fileHandle_t Plugin_FS_SV_FOpenFileWrite(
	const char *filename); // Open a file for writing to fs_homepath/filename. Needs to be closed with
						   // Plugin_FS_FCloseFile() once operation is done.
__cdecl fileHandle_t Plugin_FS_SV_FOpenFileAppend(
	const char *filename); // Open a file for appending to fs_homepath/filename. Needs to be closed with
						   // Plugin_FS_FCloseFile() once operation is done.
__cdecl long Plugin_FS_FOpenFileRead(const char *filename,
	fileHandle_t *fp); // Open a file for reading. First from fs_homepath/fs_game/filename. If not found from
					   // fs_baspath/fs_game/filename. Searches in .zip (.iwd) files. Returns length of file which
					   // should be greater than 0. Writes into *fp the filehandle. Needs to be closed with
					   // Plugin_FS_FCloseFile() once operation is done.
__cdecl fileHandle_t Plugin_FS_FOpenFileWrite(
	const char *filename); // Open a file for writing to fs_homepath/fs_game/filename . Needs to be closed with
						   // Plugin_FS_FCloseFile() once operation is done.
__cdecl fileHandle_t Plugin_FS_FOpenFileAppend(
	const char *filename); // Open a file for appending to fs_homepath/fs_game/filename . Needs to be closed with
						   // Plugin_FS_FCloseFile() once operation is done.

__cdecl int Plugin_FS_Read(void *bufferToReadIntoFromFile, int maxBytesToRead,
	fileHandle_t f); // Read data from file opened with Plugin_FS_SV_FOpenFileRead() or Plugin_FS_FOpenFileRead()
__cdecl int Plugin_FS_Write(const void *bufferToWriteIntoFile, int numBytesToWrite,
	fileHandle_t h); // Write data to a file opened with Plugin_FS_SV_FOpenFileWrite() or Plugin_FS_FOpenFileWrite().

__cdecl int Plugin_FS_ReadLine(void *buffer, int len,
	fileHandle_t
		f); // Read a line from file opened with Plugin_FS_SV_FOpenFileRead() but not others. [Better don't use]

__cdecl qboolean Plugin_FS_FCloseFile(
	fileHandle_t f); // Close an open file opened with Plugin_FS_SV_FOpenFileRead(), Plugin_FS_SV_FOpenFileWrite(),
					 // Plugin_FS_SV_FOpenFileAppend(), Plugin_FS_FOpenFileRead(), Plugin_FS_FOpenFileWrite(),
					 // Plugin_FS_FOpenFileAppend().

__cdecl int Plugin_FS_ReadFile(const char *qpath, void **buffer);
/* Reads file into a memory buffer. You have to free the buffer after usage unless you want to keep it. Read file on
   normal searchpath order. fs_homepath/fs_game/qpath, fs_basepath/fs_game/qpath, or in loaded zip files Example: char*
   buf; Plugin_FS_ReadFile("myfile.txt", (void**)&buf); Plugin_Printf("%s", buf); Plugin_FS_FreeFile(buf);
*/

// Writes the provided buffer into the file named by qpath. This is the most easiest way to write a file. File gets
// written to fs_homepath/fs_game/qpath .
...
*/
int Plugin_FS_SV_HomeWriteFile(const char *qpath, const void *buffer,
	int size); // Same as Plugin_FS_WriteFile() but file gets written to fs_homepath/qpath
int Plugin_FS_SV_BaseWriteFile(const char *qpath, const void *buffer,
	int size); // Same as Plugin_FS_WriteFile() but file gets written to fs_basepath/qpath
...
__cdecl void Plugin_AddCommandForClientToWhitelist(int clnum,
	const char *cmd); // Use this function to allow the player clnum to execute it whatever his powerpoints are
...
__cdecl void Plugin_Error(int code, const char *fmt,
	...);							 // Notify the server of an error, action depends on code parameter
...
__cdecl void Plugin_BanClient(unsigned int clientnum, int minutes, int invokerid,
	char *reason); // Bans the client for minutes from server. minutes can be "-1" to create a permanent ban. invokerid
				   // can be 0 or the numeric uid. banreason can be NULL or a valid char* pointer.

//  -- TCP Connection functions --
/*
connection is a static constant number. Every plugin can use a connection 0 up to 3. This is not a socket. This is
handled internal. You can not use the same number for 2 open connections on the same time.

!!!Attention: Behaviour has changed!!!
*/
__cdecl qboolean Plugin_TcpConnect(int connection,
	const char *remote); // Open a new TCP connection - Returns qfalse if failed, remote can be a domainname
__cdecl qboolean Plugin_TcpConnectMT(int pluginID, int connection, const char *remote); // Threadsafe needs plugin ID
__cdecl int Plugin_TcpGetData(int connection, void *buf,
	int size); // Receive TCP data - buf and size is the receiving buffer. It returns -1 if the connection is closed. It
			   // returns 0 when no new data is available. All other return values is the number of bytes received.
__cdecl int Plugin_TcpGetDataMT(int pluginID, int connection, void *buf, int size); // Threadsafe needs plugin ID
__cdecl int Plugin_TcpSendData(int connection, void *data,
	int len); // Send TCP data - buf and len point to the buffer which has the data to send. Len is the amount to bytes
			  // to send. Returns -1 if connection got closed. Returns 0 if nothing could be sent. All other values are
			  // the sent data
...
__cdecl int
Plugin_Cmd_GetInvokerUID(); // UID of commandinvoker. 0 if undefined . Deprecated in COD4x18 Do not Use this !
...
__cdecl const char *Plugin_Cmd_GetInvokerName(char *buf,
	int len); // Playername/stored name of command invoker. "" if undefined

__cdecl qboolean Plugin_CanPlayerUseCommand(int clientnum,
	const char *commandname); // Returns if the player is allowed to use this command (power)
...
__cdecl void Plugin_PlayerAddBanByip(netadr_t *remote, char *message,
	int expire); // Bans a player by ip into a temp banlist. message should be the displayed message without including
				 // times. expire is unixtime when this player is getting unbanned or -1
__cdecl char *Plugin_PlayerBannedByip(netadr_t *netadr, char *message,
	int len); // Checks if the given IP is on the banlist. Returns NULL if this player is not banned. Otherwise it is
			  // the stored message assigned to this ban.
__cdecl const char *Plugin_FormatBanMessage(int timeleftsecs, char *outbuffer, int outbufferlen, const char *reasonfmt,
	...); // function to merge the banreason and add this text "You have been banned from this server with the following
		  // reason:" includeing appealurl if available
__cdecl const char *Plugin_WriteBanTimelimit(int timeleftsecs, char *outbuffer,
	int outbufferlen); // Function to turn the remaining ban time into a text

/* blocking HTTP */
__cdecl ftRequest_t *Plugin_HTTP_Request(const char *url, const char *method, byte *requestpayload, int payloadlen,
	const char *additionalheaderlines);				   /* blocking */
__cdecl ftRequest_t *Plugin_HTTP_GET(const char *url); /* blocking */

/* non blocking HTTP */
__cdecl ftRequest_t *Plugin_HTTP_MakeHttpRequest(const char *url, const char *method, byte *requestpayload,
	int payloadlen, const char *additionalheaderlines);
__cdecl int Plugin_HTTP_SendReceiveData(ftRequest_t *request);

__cdecl void Plugin_HTTP_FreeObj(ftRequest_t *request);

__cdecl void Plugin_HTTP_CreateString_x_www_form_urlencoded(char *outencodedstring, int len, const char *key,
	const char *value);
...
__cdecl qboolean Plugin_ThreadisSame(
	threadid_t threadid); // Returns qtrue if the current calling thread has the same thread id as the argument

/* Plugin_CreateNewThread creates a new thread. ThreadMain has to be function invoked by new thread. tid can be NULL or
a pointer to a threadid_t variable returning new thread id into it. funcarg can be NULL or anything which getting passed
to thread main function Example:
